...
def get_fw_version(path_to_version_dot_c):
    """
...
def get_SRFN_fw_version(path_to_version_dot_c):
    """
    Read FW version from the version.c File

    :return: None
    """


    ifDefString = "ifdef __BOOTLOADER"
    fileAsString = open(path_to_version_dot_c, 'r').read()
    ...
    if after == "":
        raise Exception("The format of the version.c file is wrong!")


    _before, _tag, after = after.partition("BOOTLOADER_VER")
    _before, _tag, after = after.partition("t)")
    ver, _tag, after = after.partition(") ")
    bootloader_ver = int(ver.strip())
    print(f"bootloader_ver={bootloader_ver}")

    _before, _tag, after = after.partition("BOOTLOADER_REV")
    _before, _tag, after = after.partition("t)")
    rev, _tag, after = after.partition(") ")
    bootloader_rev = int(rev.strip())
    print(f"bootloader_rev={bootloader_rev}")

    _before, _tag, after = after.partition("BOOTLOADER_BUILD")
    _before, _tag, after = after.partition("t)")
    build, _tag, after = after.partition(") ")
    bootloader_build = int(build.strip())
    print(f"bootloader_build={bootloader_build}")

    set_file_global(name='bootloader_ver', value=bootloader_ver)
    set_file_global(name='bootloader_rev', value=bootloader_rev)
    set_file_global(name='bootloader_build', value=bootloader_build)

    bootloader_version_string = "%02d.%02d.%04d" % (bootloader_ver, bootloader_rev, bootloader_build)
    set_file_global(name='bootloader_version_string', value=bootloader_version_string)

    ifDefString = "else //Application version"

    _before, _tag, after = after.partition("FIRMWARE_VER")
    _before, _tag, after = after.partition("t)")
    ver, _tag, after = after.partition(") ")
    firmware_ver = int(ver.strip())
    print(f"firmware_ver={firmware_ver}")

    _before, _tag, after = after.partition("FIRMWARE_REV")
    _before, _tag, after = after.partition("t)")
    rev, _tag, after = after.partition(") ")
    firmware_rev = int(rev.strip())
    print(f"firmware_rev={firmware_rev}")

    _before, _tag, after = after.partition("FIRMWARE_BUILD")
    _before, _tag, after = after.partition("t)")
    build, _tag, after = after.partition(") ")
    firmware_build = int(build.strip())
    print(f"firmware_build={firmware_build}")


    set_file_global(name='firmware_ver', value=firmware_ver)
    set_file_global(name='firmware_rev', value=firmware_rev)
    set_file_global(name='firmware_build', value=firmware_build)

    firmware_version_string = "%02d.%02d.%04d" % (firmware_ver, firmware_rev, firmware_build)
    set_file_global(name='firmware_version_string', value=firmware_version_string)
...
class ArtifactFolders:
    def __init__(self, project_path, release_name, firmware_family, version_string):
        ...
        # set_file_global(name='build_dir_name', value=self.build_dir_name)
        build_package = f"{release_name}_{version_string}_{current_datetime_str}"
        set_file_global(name='build_package', value=build_package)
