...
<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<AUTO GENERATED BY CONFLICT EXTENSION<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< Development/SRFN-EP_MCU_Replacement_Dev/SRFN-EP_MCU_Replacement_Dev_Main_Dev
====================================AUTO GENERATED BY CONFLICT EXTENSION====================================
   INIT( FIO_finit, STRT_FLAG_LAST_GASP ),                                          // This must be after CRC_initialize because it uses CRC.
   INIT( MFGP_cmdInit, (STRT_FLAG_QUIET|STRT_FLAG_RFTEST) ),
>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>AUTO GENERATED BY CONFLICT EXTENSION>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> Development/SRFN-EP_MCU_Replacement_Dev/SRFN-EP_MCU_Replacement_Dev_Uart
#if 0
