...
/* MACRO DEFINITIONS */
   ...
   #define BOOTLOADER_BUILD  ((uint16_t)7)   /* current bootloader build */
   ...
   #define FIRMWARE_BUILD  ((uint16_t)158)   /* current firmware build */ // Don't put a '0' in front of the rev number. It's going to be interpreted as an octal number and might not build.
#endif
...
#elif ( ( HAL_TARGET_HARDWARE == HAL_TARGET_Y84580_x_REV_A ) || ( HAL_TARGET_HARDWARE == HAL_TARGET_Y84580_x_REV_B ) )
