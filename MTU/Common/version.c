...
/* MACRO DEFINITIONS */
   ...
   #define FIRMWARE_VER    ((uint8_t)2)      /* current firmware version */
   #define FIRMWARE_REV    ((uint8_t)0)      /* current firmware revision */
   #define FIRMWARE_BUILD  ((uint16_t)80)    /* current firmware build */ // Don't put a '0' in front of the rev number. It's going to be interpreted as an octal number and might not build.
#endif
