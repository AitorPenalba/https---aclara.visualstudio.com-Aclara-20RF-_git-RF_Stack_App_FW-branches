...
{ /* Body */
   ...
   while ((c = *format++) != 0) {
      ...
      if ( c != '%' ) {
         ...
         /* Aclara added to support LL and HH format specifiers */
         /*
          * Now check to see if c is an additional type specifier.
          */
         if (c  == 'H') {
            numtype = SCAN_BLONG;
            if ( ! _io_scanline_format_ignore_white_space(
...
print_val:
               if (  ! suppress_field  ) {
                  ...
                  switch ( numtype ) {
                     /* Aclara added to support LL format specifiers */
                     case SCAN_LLLONG:
                        *((uint64_t *)tmp_ptr) = val;
                        break;
                     /* End Aclara add */
                     case SCAN_LLONG:
...
string_done:
         ...
         case 'E':
            ...
            /* Aclara Added to next line to do nothing if LL float format specifier - 80 bit floats not supported */
            else if (SCAN_LLLONG != numtype)
            {
