...
{ /* Body */
    ...
    // Compute how long the previous task has been running
    if ( kernel_data->OLD_ACTIVE_PTR && (kernel_data->OLD_ACTIVE_PTR->TASK_ID != TASK_ID_DYING) ) {
       kernel_data->OLD_ACTIVE_PTR->RUN_TIME += DWT_CYCCNT-kernel_data->OLD_ACTIVE_PTR->START_TIME;
    }   

    // Update start time for this task
    kernel_data->ACTIVE_PTR->START_TIME = DWT_CYCCNT;
