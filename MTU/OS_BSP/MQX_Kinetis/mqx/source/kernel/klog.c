 ...
 * units) of stack used.
 * \param[out] stack_limit_ptr Where to write the limit (in single-addressable
 * units) of the stack.  //Aclara Added
 *
 * \return MQX_OK
...
(
    ...
    _mem_size_ptr stack_limit_ptr   /* Aclara Add */
)
{ /* Body */
...
#endif
    /* Aclara Add stack_limit_ptr parameter */
    return _klog_get_task_stack_usage_internal(td_ptr, stack_size_ptr, stack_used_ptr, stack_limit_ptr);
 ...
 * \param[out] stack_limit_ptr End of stack.    //Aclara add
 *
 * \return MQX_OK
 * \return MQX_INVALID_TASK_ID (Task_id is not valid.)
 * \return MQX_INVALID_CONFIGURATION (Compile-time configuration option
...
{ /* Body */
    ...
    *stack_size_ptr = (_mem_size)((unsigned char *)td_ptr->STACK_LIMIT -
                    (unsigned char *)td_ptr->STACK_BASE);
#endif
    *stack_limit_ptr = (_mqx_uint)td_ptr->STACK_LIMIT;   /* Aclara Add */
    return(MQX_OK);
#else
    return MQX_INVALID_CONFIGURATION;
#endif
...
// Aclara Add. Convert a float value into a string
// Harcoded for small positive number with 1 decimal
char * printFloat(float f)
{
   ...
   integer = (int32_t)f;
   fraction = (uint32_t)((f-(float)integer)*10.0);
   ...
   // Add fractional part
   (void)sprintf(&str[strlen(str)], "%01u", fraction);
...
{ /* Body */
    ...
    _mem_size              stack_limit;   // Aclara Add
    _mqx_uint              size;
    ...
    printf("Stack usage: \n\n");
    /* Aclara Add used % */
    printf("Interrupt stack:    size %08ld    used %08ld used(%%)%4s\n\n",
                    (uint32_t)stack_size, (uint32_t)stack_used, printFloat(stack_used*100.0/stack_size));

    printf("SIZE                  USED                 USED(%%) TASK ID      NAME\n");
    ...
    while (size && td_ptr)
    { /* Aclara Add used %*/
        _klog_get_task_stack_usage_internal(td_ptr, &stack_size, &stack_used, &stack_limit);
        printf("%08ld(%08lX)    %08ld(%08lX)   %4s    %08lX     %s\n",
                        (uint32_t)stack_size, (uint32_t)stack_size, (uint32_t)stack_used,
                        (uint32_t)stack_used, printFloat(stack_used*100.0/stack_size), td_ptr->TASK_ID,
...
#if MQX_KERNEL_LOGGING  /* Aclara Addd */
/*!
