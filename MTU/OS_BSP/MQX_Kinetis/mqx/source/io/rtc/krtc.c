...
{ /* Body */
    ...
    {
        //Aclara Mar-16-2016 Set default load cap, CLKO, WPS, UM, SUPWPE & SWR at default
        rtc->CR  = RTC_CR_SCxP_DEFAULT;
    ...
    //Aclara Mar-16-2016 Set the crytal load capacitance if not set
    if ( 0 == ( rtc->CR & ( RTC_CR_SC16P_MASK | RTC_CR_SC8P_MASK | RTC_CR_SC4P_MASK | RTC_CR_SC2P_MASK ) ) )
    {
        rtc->SR &= ~RTC_SR_TCE_MASK;      /* disable rtc timer - enabling write access */
        rtc->CR &= ~RTC_CR_OSCE_MASK;     /* Disable OSC */
        rtc->CR  = RTC_CR_SCxP_DEFAULT;   /* set default load cap, CLKO, WPS, UM, SUPWPE & SWR at default */
        rtc->CR |= RTC_CR_OSCE_MASK;      /* Enable OSC */
        /* recommended 125 ms delay for oscillator start */
        _time_delay(125);
        rtc->SR |= RTC_SR_TCE_MASK;       /* enable rtc timer */
    }
    return result;
} /* Endbody */
...
{ /* Body */
    ...
    *timep = (uint32_t)rtc->TSR;
    //Aclara Feb-26-2016 Kxx Ref Manual says to do this to ensure register is read correctly
    while ( *timep != (uint32_t)rtc->TSR )
    {
        *timep = (uint32_t)rtc->TSR;
    }
...
//Aclara Feb-22-2017 Added _rtc_get_prescale to read TPR register for selftest
/*FUNCTION****************************************************************
*
* Function Name    : _rtc_get_prescale
* Returned Value   : MQX_OK
* Comments         :
*    This function gets the RTC Time Prescaler register.
*
*END*********************************************************************/
int32_t _rtc_get_prescale
(
    uint16_t *prescalep
)
{ /* Body */
    RTC_MemMapPtr rtc = RTC_BASE_PTR;
    *prescalep = (uint16_t)rtc->TPR;
    //Aclara Feb-26-2016 Kxx Ref Manual says to do this to ensure register is read correctly
    while ( *prescalep != (uint16_t)rtc->TPR )
    {
        *prescalep = (uint16_t)rtc->TPR;
    }
...
{ /* Body */
    RTC_MemMapPtr rtc = RTC_BASE_PTR;
    //Aclara Feb-26-2016 Add to ensure correct read of TSR register
    uint32_t      tsr;

    _rtc_get_time(&tsr);
    if (time < tsr) /* Alarm time must be larger than current time */
    {
