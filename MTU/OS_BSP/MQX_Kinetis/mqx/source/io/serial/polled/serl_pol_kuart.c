...
{ /* Body */
   ...
   switch (cmd) {
      ...
      /* Next two commands added 1/10/2017 to support Optical port communications. With the TX enabled, one of the comm
         lines between the optical port and the meter is forced high, which effectivel disables the optical port.   */
      case IO_IOCTL_SERIAL_DISABLE_TX:
            /* disable transmitter */
            sci_ptr->C2 &=~UART_C2_TE_MASK;
         break;
      case IO_IOCTL_SERIAL_ENABLE_TX:
            /* enable transmitter */
            sci_ptr->C2 |= UART_C2_TE_MASK;
         break;
      ...
      case IO_IOCTL_SERIAL_GET_CONFIG:
      {
         ...
         *io_init_ptr = io_info_ptr->INIT;
         break;
          ...
          //Aclara Added 2015-Mar-19 to Support 9-bit mode
      case IO_IOCTL_SERIAL_GET_ADDRESS_DEVICE:
         *param_ptr++ = sci_ptr->MA1;
         *param_ptr   = sci_ptr->MA2;
         break;
          //Aclara Added 2015-Mar-19 to Support 9-bit mode
      case IO_IOCTL_SERIAL_SET_ADDRESS_DEVICE:
         sci_ptr->MA1 = UART_MA1_MA(*param_ptr++);
         sci_ptr->MA2 = UART_MA2_MA(*param_ptr);
         break;
          //Aclara Added 2015-Mar-19 to Support 9-bit mode
      case IO_IOCTL_SERIAL_GET_MA_ENABLE:
         *param_ptr++ = sci_ptr->C4 & UART_C4_MAEN1_MASK ? 1 : 0;
         *param_ptr   = sci_ptr->C4 & UART_C4_MAEN2_MASK ? 1 : 0;
         break;
          //Aclara Added 2015-Mar-19 to Support 9-bit mode
      case IO_IOCTL_SERIAL_SET_MA_ENABLE:
         if( *(bool *)param_ptr == TRUE )
         {
            /* enable Receiver Match Address Mode */
            sci_ptr->C4 |= UART_C4_MAEN1_MASK;
         }
         else
         {
            /* disable Receiver Match Address Mode */
            sci_ptr->C4 &=~UART_C4_MAEN1_MASK;
         }
         param_ptr++;
         if( *(bool *)param_ptr == TRUE )
