...
{
    ADC_MemMapPtr           adc_ptr;
    //Aclara Mar-06-2015 Removed Continuous Conversions (ADC_SC3_ADCO_MASK)
    uint32_t                mcr = LWADC_SC3_AVGS_32 | ADC_SC3_AVGE_MASK;
    ...
    //Aclara Mar-03-2015 Add unsupported controls
    if (init_ptr->SPEED == LWADC_HSC_HIGH)
    {
      adc_ptr->CFG2 |= ADC_CFG2_ADHSC_MASK;
    }
    if (init_ptr->POWER == LWADC_LPC_LOW)
    {
      adc_ptr->CFG1 |= ADC_CFG1_ADLPC_MASK;
    }
    //Aclara Mar-03-2015 end
