...
{ /* Body */
   ...
   _int_disable ();
   i2c_ptr->C1 &= ~I2C_C1_IICIE_MASK;  //Aclara Add disable periph interrupt instead of _bsp_int_disable
   //Aclara _bsp_int_disable (io_info_ptr->VECTOR);
   _int_enable ();
   ...
   i2c_ptr->C1 |= I2C_C1_IICIE_MASK;  //Aclara Add disable periph interrupt instead of _bsp_int_enable
   //Aclara _bsp_int_enable (io_info_ptr->VECTOR);

   /* Wait for rx complete */
   // Aclara change; don't wait FOREVER for semaphore. If time out in .5 second, abort with "Lost arbitration"
   if ( MQX_OK != _lwsem_wait_ticks((LWSEM_STRUCT_PTR)(&(io_info_ptr->LWSEM)), 100) )
   {
...
{ /* Body */
   ...
   if (I2C_MODE_MASTER == io_info_ptr->MODE)
   {
      ...
      else if(I2C_STATE_TRANSMIT == tmp)
      {
         ...
         {
            /* Aclara Added _bsp_int_enable() to keep interrupts enabled for the STOPF */
            i2c_ptr->C1 |= I2C_C1_IICIE_MASK;  //Aclara Add enable periph interrupt instead of _bsp_int_enable
            //Aclara  _bsp_int_enable (io_info_ptr->VECTOR);
            return 0;
         }
      }
...
{ /* Body */
   ...
   if (i2c_ptr->C1 & I2C_C1_MST_MASK)
   {
      ...
      if (i2c_ptr->C1 & I2C_C1_TX_MASK)
      {
         ...
         if (i2csr & I2C_S_RXAK_MASK)
         {
            ...
            /* Aclara Removed to keep interrupts enabled for the STOPF
   ...
   {
      /* Aclara No interrupt disable changes like above in this block since not running as Slave */
