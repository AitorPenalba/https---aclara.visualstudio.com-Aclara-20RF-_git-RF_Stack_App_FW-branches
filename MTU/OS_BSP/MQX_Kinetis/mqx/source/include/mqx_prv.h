...
typedef struct   td_struct
{
   ...
   /*! \brief When the task was started by the scheduler. */
   _mqx_uint                         START_TIME;

   /*! \brief Accumulated time that this task has been running. */
   _mqx_uint                         RUN_TIME;

   /*! \brief Accumulated time the this task has been in an interrupt. */
   _mqx_uint                         INT_TIME;
...
extern _mqx_uint     _klog_get_task_stack_usage_internal(TD_STRUCT_PTR,
   _mem_size_ptr, _mem_size_ptr, _mem_size_ptr);   /* Aclara Add _mem_size_ptr */
