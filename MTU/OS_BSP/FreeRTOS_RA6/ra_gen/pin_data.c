...
const ioport_pin_cfg_t g_bsp_pin_cfg_lpm_data[] = {
    {
        .pin = BSP_IO_PORT_00_PIN_00,
        .pin_cfg = ((uint32_t) IOPORT_CFG_PORT_DIRECTION_INPUT)
    },
...
const ioport_cfg_t g_bsp_pin_cfg_lpm = {
    .number_of_pins = sizeof(g_bsp_pin_cfg_lpm_data)/sizeof(ioport_pin_cfg_t),
    .p_pin_cfg_data = &g_bsp_pin_cfg_lpm_data[0],
};
...
void R_BSP_PinCfgSecurityInit(void)
{
    ...
    for(uint32_t i = 0; i < g_bsp_pin_cfg_lpm.number_of_pins; i++)
    {
        uint32_t port_pin = g_bsp_pin_cfg_lpm.p_pin_cfg_data[i].pin;
        uint32_t port = port_pin >> 8U;
        uint32_t pin = port_pin & 0xFFU;
        pmsar[port] &= (uint16_t) ~(1U << pin);
    }
