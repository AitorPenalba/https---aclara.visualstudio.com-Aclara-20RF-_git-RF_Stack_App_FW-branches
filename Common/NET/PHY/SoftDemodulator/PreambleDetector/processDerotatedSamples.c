...
#define TIMING_LOOP_THRESHOLD 48 // Must be an interger.

// Used in both preambleDetector.c and SyncAndPayloadDemodulator.c
                                 ...
                                 int      *c_ZCerrAcc)
{
   ...
   */
   ...
   for (i = startingIndex - 1; i < samplesInBuffer; i++)
   {
   ...
   while (samplesToProcess > 13)
   {
      ...
      for (i = 0; i < 8; i++)
      {
         ...
         if ((iv0[i] + 1) == LoP)
         {
            c_findSyncStartIndex = *c_SlicOutIdx;

            // findSYNC searches for SYNC starting from (findSyncStartIndex -32) onwards.
         }
      }
      ...
      if (*c_ZCerrAcc <= -TIMING_LOOP_THRESHOLD)
      {
         jxAdj = 7;

         /*  ZCerr ; % -1; % Using jxAdj = ZCerr will increase BW. */
         *c_ZCerrAcc += TIMING_LOOP_THRESHOLD;
      }
      else if (*c_ZCerrAcc >= TIMING_LOOP_THRESHOLD)
      {
         jxAdj = 9;
