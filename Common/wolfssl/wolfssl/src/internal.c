...
static int DoVerifyCallback(WOLFSSL* ssl, int ret, ProcPeerCertArgs* args)
{
    ...
    if (use_cb && ssl->verifyCallback) {
        ...
        #if defined(OPENSSL_EXTRA) || defined(OPENSSL_EXTRA_X509_SMALL) || 1
        WOLFSSL_X509* x509;
        #endif
        char* domain = NULL;
    #else
                          ...
                          word32 totalSz)
{
    ...
    if (ssl->keys.dtls_peer_handshake_number >
        ...
        *inOutIdx += fragSz;
        ...
        if (ret == 0) {
			/***
			* Change made by Aclara - since we aren't sending the Hello Verify, we
			* won't get that response which would have incremented the expected peer
			* handshake number (this number is actually the expected state, which is
			* sequential.) We need to move past that state when we process client hello
			* to indicate that we aren't expecting that response. This number was not
			* incremented for a client hello message when not in Sctp Mode. Since
			* we're incrementing for client hello as well as all other messages,
			* no if test is necessary.
			***/
...
#endif
    WOLFSSL_ERROR(*type);
    if (IsEncryptionOn(ssl, 0) && ssl->keys.decryptedCur == 0) {	/* CJM - added this check ( ssl->keys.decryptedCur == 0) for alerts from epoch 0 received after connection established*/
        *inOutIdx += ssl->keys.padSz;
	}
    return level;
}
    ...
    word32 size)
{
    ...
    *inOutIdx += length;
                             ...
                             word32 helloSz)
    {
        ...
        if (IsDtlsNotSctpMode(ssl)) {
			/***
			* Modified by Aclara. The wolfSSL 3.6.0 library did not handle the hello verify, and
			* that's the version for the firmware in the field. 
			***/
#ifdef WOLFSSL_SEND_HELLO_VERIFY
