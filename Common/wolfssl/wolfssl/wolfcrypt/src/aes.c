...
static byte roll[AES_BLOCK_SIZE];
static byte bkey[AES_BLOCK_SIZE];

int wc_AesCcmVerifyDecrypt(Aes* aes, const byte* in, word32 inSz, const byte* authTag, word32 authTagSz, word32 nonceSz, byte lastCall)
{
  int i;
  int result = 0;

  byte lenSz = AES_BLOCK_SIZE - 1 - (byte)nonceSz;

  roll_x(aes, in, inSz, roll);
  ...
  if (lastCall)
  {
    ...
    for (i = 0; i < lenSz; i++)
    {
      bkey[AES_BLOCK_SIZE - 1 - i] = 0;
    }

    wc_AesEncrypt(aes, bkey, bkey);

    xorbuf(roll, bkey, authTagSz);

    if (ConstantCompare(roll, authTag, authTagSz) != 0) {
        /* If the authTag check fails, don't keep the decrypted data.
         * Unfortunately, you need the decrypted data to calculate the
         * check value. */
        result = AES_CCM_AUTH_E;
    ...
    ForceZero(roll, AES_BLOCK_SIZE);
    ForceZero(bkey, AES_BLOCK_SIZE);
  }

  return result;
}

void  wc_AesCcmDecryptNoValidate(Aes* aes, byte* out, const byte* in, word32 inSz,
                                const byte* nonce, word32 nonceSz,
                                const byte* authTag, word32 authTagSz,
                                const byte* authIn, word32 authInSz,
                                word32 totalSz, byte initialize, byte lastCall)
{
    byte A[AES_BLOCK_SIZE];
    byte* o;
    byte lenSz;
    ...
    if (initialize)
    {
        ...
        XMEMCPY(bkey+1, nonce, nonceSz);

        bkey[0] = lenSz - 1;
        for (i = 0; i < lenSz; i++)
          bkey[AES_BLOCK_SIZE - 1 - i] = 0;
        bkey[AES_BLOCK_SIZE - 1] = 1;
    }

    while (oSz >= AES_BLOCK_SIZE) {
        #ifdef FREESCALE_MMCAU
            cau_aes_encrypt(bkey, key, aes->rounds, A);
        #else
            wc_AesEncrypt(aes, bkey, A);
        #endif
        xorbuf(A, in, AES_BLOCK_SIZE);
        XMEMCPY(o, A, AES_BLOCK_SIZE);

        AesCcmCtrInc(bkey, lenSz);
        oSz -= AES_BLOCK_SIZE;
        in += AES_BLOCK_SIZE;
        o += AES_BLOCK_SIZE;
    ...
    if (lastCall)
    {
       ...
       /* Step 1 initalize the roll */
       for (i = 0; i < lenSz; i++)
         bkey[AES_BLOCK_SIZE - 1 - i] = 0;
       wc_AesEncrypt(aes, bkey, roll);

       /* Step 2 start the roll off */
       bkey[0] = (authInSz > 0 ? 64 : 0)
         + (8 * (((byte)authTagSz - 2) / 2))
         + (lenSz - 1);
       for (i = 0; i < lenSz; i++)
         bkey[AES_BLOCK_SIZE - 1 - i] = (totalSz >> (8 * i)) & 0xFF;
       wc_AesEncrypt(aes, bkey, roll);
    }
    return;
}
