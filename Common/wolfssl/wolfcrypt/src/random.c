    ...
    if (wc_RNG_HealthTestLocal(0) == 0) {
        ...
        if (rng->drbg == NULL) {
            ...
            if (ret != 0)
                ...
                ret = wc_RNG_TestSeed(seed, seedSz);	//Ming: Code for testing RNG seed (to pass the test in DtlsGenerateSeed() we need call ecc108e_SecureRandom() twice ). Comment out if we dont't want it.
