 ...
 *****************************************************************************/
DESC_CALLBACK_FUNCTIONS_STRUCT  desc_callback = /*lint !e765 could be made static   */
{
   0xFF,
...
USB_EP_STRUCT ep[CDC_DESC_ENDPOINT_COUNT] =  /*lint !e765 could be made static   */
{
...
USB_ENDPOINTS usb_desc_ep =   /*lint !e765 could be made static   */
{
   CDC_DESC_ENDPOINT_COUNT,
   ep
};

uint8_t g_device_descriptor[DEVICE_DESCRIPTOR_SIZE] = /*lint !e765 could be made static   */
{
   ...
   0x28, 0x0d,                         /* Vendor ID - NXP   */
/* 0x25, 0x04,                            Vendor ID - Freescale (Motorola) */
...
uint8_t g_config_descriptor[CONFIG_DESC_SIZE] = /*lint !e765 could be made static   */
{
   ...
   BUS_POWERED | SELF_POWERED | ( REMOTE_WAKEUP_SUPPORT << REMOTE_WAKEUP_SHIFT ), /*  Attributes.support RemoteWakeup and self power */
...
/*lint -esym(553,HIGH_SPEED_DEVICE) not defined, assumed 0 */
...
uint8_t  g_other_speed_config_descriptor[OTHER_SPEED_CONFIG_DESCRIPTOR_SIZE] =
{
   ...
   USB_OTHER_SPEED_DESCRIPTOR,         /* This is a Other speed config descr */
   USB_uint_16_low( CONFIG_DESC_SIZE ), /*  Total length of the Configuration descriptor */
   USB_uint_16_high( CONFIG_DESC_SIZE ),
   CONFIG_DESC_NUM_INTERFACES_SUPPOTED,
   1,                                  /*value used to selct this configuration : Configuration Value */
...
uint8_t USB_STR_0[USB_STR_0_SIZE + USB_STR_DESC_SIZE] =  /*lint !e765 could be made static   */
{
   sizeof( USB_STR_0 ),
...
uint8_t USB_STR_1[USB_STR_1_SIZE + USB_STR_DESC_SIZE] =  /*lint !e765 could be made static   */
{
   sizeof( USB_STR_1 ),
   USB_STRING_DESCRIPTOR,
   'N', 0,
...
uint8_t USB_STR_2[USB_STR_2_SIZE + USB_STR_DESC_SIZE] =  /*lint !e765 could be made static   */
{
   sizeof( USB_STR_2 ),
   USB_STRING_DESCRIPTOR,
   'M', 0,
...
uint8_t USB_STR_n[USB_STR_n_SIZE + USB_STR_DESC_SIZE] =  /*lint !e765 could be made static   */
{
   sizeof( USB_STR_n ),
   USB_STRING_DESCRIPTOR,
   'B', 0,
...
uint32_t g_std_desc_size[USB_MAX_STD_DESCRIPTORS + 1] =  /*lint !e765 could be made static   */
{
   0,
...
uint8_t *g_std_descriptors[USB_MAX_STD_DESCRIPTORS + 1] =   /*lint !e765 could be made static   */
{
   NULL,
   g_device_descriptor,
   g_config_descriptor,
...
uint8_t g_string_desc_size[USB_MAX_STRING_DESCRIPTORS + 1] =   /*lint !e765 could be made static   */
{
   sizeof( USB_STR_0 ),
   sizeof( USB_STR_1 ),
   sizeof( USB_STR_2 ),
...
uint8_t *g_string_descriptors[USB_MAX_STRING_DESCRIPTORS + 1] =   /*lint !e765 could be made static   */
{
   USB_STR_0,
   USB_STR_1,
   USB_STR_2,
...
USB_LANGUAGE    usb_lang[USB_MAX_SUPPORTED_INTERFACES] = /*lint !e765 could be made static   */
{
   ( uint16_t )0x0409,
   g_string_descriptors,
   g_string_desc_size
};
USB_ALL_LANGUAGES g_languages =  /*lint !e765 could be made static   */
{
   USB_STR_0, sizeof( USB_STR_0 ),
   usb_lang
};

uint8_t g_valid_config_values[USB_MAX_CONFIG_SUPPORTED + 1] = {0, 1};   /*lint !e765 could be made static   */

uint8_t g_line_coding[LINE_CODING_SIZE] = /*lint !e765 could be made static   */
{
   /*e.g. 0x00,0x10,0x0E,0x00 : 0x000E1000 is 921600 bits per second */
   ( (uint32_t)LINE_CODE_DTERATE_IFACE >> 0 )   & 0x000000FF,  /*lint !e778 expression evaluates to 0 */
   ( (uint32_t)LINE_CODE_DTERATE_IFACE >> 8 )   & 0x000000FF,  /*lint !e778 expression evaluates to 0 */
   ( (uint32_t)LINE_CODE_DTERATE_IFACE >> 16 )  & 0x000000FF,  /*lint !e778 expression evaluates to 0 */
   ( (uint32_t)LINE_CODE_DTERATE_IFACE >> 24 )  & 0x000000FF,  /*lint !e572 !e778 excessive shift, evaluates to 0 */
   LINE_CODE_CHARFORMAT_IFACE,
   LINE_CODE_PARITYTYPE_IFACE,
   LINE_CODE_DATABITS_IFACE
};

uint8_t g_abstract_state[COMM_FEATURE_DATA_SIZE] = /*lint !e765 could be made static   */
{
   ( STATUS_ABSTRACT_STATE_IFACE >> 0 ) & 0x00FF,  /*lint !e572 excessive shift */
   ( STATUS_ABSTRACT_STATE_IFACE >> 8 ) & 0x00FF   /*lint !e572 excessive shift */
};

uint8_t g_country_code[COMM_FEATURE_DATA_SIZE] =   /*lint !e765 could be made static   */
{
   ( COUNTRY_SETTING_IFACE >> 0 ) & 0x00FF,  /*lint !e572 excessive shift */
   ( COUNTRY_SETTING_IFACE >> 8 ) & 0x00FF   /*lint !e572 excessive shift */
};
 ...
 *****************************************************************************/
uint8_t USB_Desc_Get_Descriptor( /*lint !e765 could be made static   */
   uint32_t handle,
   uint8_t type,
   uint8_t str_num,
   uint16_t index,
...
/**************************************************************************//*!
   ...
   @return USB_OK                              When Successfull
           USBERR_INVALID_REQ_TYPE             when Error
 *****************************************************************************/
uint8_t USB_Desc_Get_Interface( uint32_t handle,   /*lint !e765 could be made static   */
                                uint8_t interface,
...
/**************************************************************************//*!
   ...
   @return USB_OK                              When Successfull
           USBERR_INVALID_REQ_TYPE             when Error
 *****************************************************************************/
uint8_t USB_Desc_Set_Interface( uint32_t handle,   /*lint !e765 could be made static   */
                                uint8_t interface,
                                uint8_t alt_interface )
{
   UNUSED_ARGUMENT( handle )
...
/**************************************************************************//*!
   ...
   @return TRUE           When Valid
           FALSE          When Error
 *****************************************************************************/
bool USB_Desc_Valid_Configation( uint32_t handle,  /*lint !e765 could be made static   */
                                 uint16_t config_val )
{
 ...
 *****************************************************************************/
bool USB_Desc_Remote_Wakeup( uint32_t handle )  /*lint !e765 could be made static   */
{
   UNUSED_ARGUMENT( handle )
 ...
 *****************************************************************************/
USB_ENDPOINTS *USB_Desc_Get_Endpoints( uint32_t handle ) /*lint !e765 could be made static   */
{
   UNUSED_ARGUMENT( handle )
   return &usb_desc_ep;
}
...
/**************************************************************************//*!
   ...
   @return USB_OK                              When Successfull
           USBERR_INVALID_REQ_TYPE             when Error
 *****************************************************************************/
uint8_t USB_Desc_Get_Line_Coding( uint32_t handle, /*lint !e765 could be made static   */
                                  uint8_t interface,
                                  uint8_t **coding_data )
{
   UNUSED_ARGUMENT( handle )
...
/**************************************************************************//*!
   ...
   @return USB_OK                              When Successfull
           USBERR_INVALID_REQ_TYPE             when Error
 *****************************************************************************/
uint8_t USB_Desc_Set_Line_Coding( uint32_t handle, /*lint !e765 could be made static   */
                                  uint8_t interface,
                                  uint8_t **coding_data )   /*lint !e818 could be ptr to const  */
{
   uint8_t count;
...
/**************************************************************************//*!
   ...
   @return USB_OK                              When Successfull
           USBERR_INVALID_REQ_TYPE             when Error
 *****************************************************************************/
uint8_t USB_Desc_Get_Abstract_State( uint32_t handle, /*lint !e765 could be made static   */
                                     uint8_t interface,
                                     uint8_t **feature_data )  /*lint !e818 could be ptr to const  */
{
   UNUSED_ARGUMENT( handle )
   /* if interface valid */
   if( interface < USB_MAX_SUPPORTED_INTERFACES )
...
/**************************************************************************//*!
   ...
   @return USB_OK                              When Successfull
           USBERR_INVALID_REQ_TYPE             when Error
 *****************************************************************************/
uint8_t USB_Desc_Get_Country_Setting( uint32_t handle,   /*lint !e765 could be made static   */
                                      uint8_t interface,
...
/**************************************************************************//*!
   ...
   @return USB_OK                              When Successfull
           USBERR_INVALID_REQ_TYPE             when Error
 *****************************************************************************/
uint8_t USB_Desc_Set_Abstract_State( uint32_t handle, /*lint !e765 could be made static   */
                                     uint8_t interface,
                                     uint8_t **feature_data )  /*lint !e818 could be ptr to const  */
{
   uint8_t count;
...
/**************************************************************************//*!
   ...
   @return USB_OK                              When Successfull
           USBERR_INVALID_REQ_TYPE             when Error
 *****************************************************************************/
uint8_t USB_Desc_Set_Country_Setting( uint32_t handle,   /*lint !e765 could be made static   */
                                      uint8_t interface,
                                      uint8_t **feature_data ) /*lint !e818 could be ptr to const  */
{
   uint8_t count;
 ...
 *****************************************************************************/
uint8_t USB_Desc_Get_feature( uint32_t handle, int32_t cmd, uint8_t in_data, uint8_t ** out_buf )  /*lint !e765 could be made static   */
{
 ...
 *****************************************************************************/
uint8_t USB_Desc_Set_feature( uint32_t handle, int32_t cmd, uint8_t in_data, uint8_t ** in_buf )   /*lint !e765 could be made static   */
{
   uint8_t error = USBERR_INVALID_REQ_TYPE;
