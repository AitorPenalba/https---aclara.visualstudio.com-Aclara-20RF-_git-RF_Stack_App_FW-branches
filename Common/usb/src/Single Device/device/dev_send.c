...
)
{
   ...
   ********************************************************/
   ...
   if ( ( ( USB_DEV_CALLBACK_FUNCTIONS_STRUCT_PTR ) usb_dev_ptr->CALLBACK_STRUCT_PTR )->DEV_SEND != NULL )
   {
      error = ( ( USB_DEV_CALLBACK_FUNCTIONS_STRUCT_PTR ) usb_dev_ptr->CALLBACK_STRUCT_PTR )->DEV_SEND( handle, xd_ptr );
   }
   else
   {
#if _DEBUG
