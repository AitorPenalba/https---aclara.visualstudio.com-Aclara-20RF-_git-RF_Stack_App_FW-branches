 ...
 *****************************************************************************/
static uint8_t USB_Audio_Other_Requests( USB_SETUP_STRUCT * setup_packet, uint8_t **data, uint32_t *size, void   *arg );
...
static void USB_Service_Audio_Status_Interrupt( PTR_USB_EVENT_STRUCT event, void *arg )
{
   ...
   if( 0 != audio_obj_ptr->ep[index].queue_num )
   {
      ...
      ( void )USB_Class_Send_Data( audio_obj_ptr->class_handle, queue.channel, queue.app_data.data_ptr, queue.app_data.data_size );
   }
#endif
   ...
   if ( audio_obj_ptr->audio_class_callback.callback != NULL )
   {
      ...
      audio_obj_ptr->audio_class_callback.callback( USB_APP_SEND_COMPLETE, 0, audio_obj_ptr->audio_class_callback.arg );
   }
}
...
static void USB_Service_Audio_Isochronous_IN( PTR_USB_EVENT_STRUCT event, void *arg )
{
   ...
   if( audio_obj_ptr->param_callback.callback != NULL )
   {
      ...
      audio_obj_ptr->param_callback.callback( event_type, ( void* )&iso_in_recv, audio_obj_ptr->param_callback.arg );
   }
}
...
static void USB_Service_Audio_Isochronous_OUT( PTR_USB_EVENT_STRUCT event, void *arg )
{
   ...
   if( audio_obj_ptr->param_callback.callback != NULL )
   {
      audio_obj_ptr->param_callback.callback( event_type, ( void* )&iso_out_recv, audio_obj_ptr->param_callback.arg );
   }
}
...
static void USB_Class_Audio_Event( uint8_t event, void* val, void *arg )
{
   ...
   if( event == USB_APP_ENUM_COMPLETE )
   {
      ...
      while( count < usb_ep_data->count )
      {
         ...
         ( void )_usb_device_init_endpoint( audio_obj_ptr->controller_handle, ep_struct_ptr, TRUE );

         /* register callback service for Non Control EndPoints */
         switch( ep_struct_ptr->type )
         {
            case USB_INTERRUPT_PIPE :
               ( void )_usb_device_register_service( audio_obj_ptr->controller_handle, ( uint8_t )( USB_SERVICE_EP0 + ep_struct_ptr->ep_num ),
         ...
         ( void )_usb_device_set_status( audio_obj_ptr->controller_handle, ( uint8_t )( USB_STATUS_ENDPOINT | ep_struct_ptr->ep_num ), ( uint8_t )USB_STATUS_IDLE );
         count++;
      }/* EndWhile */
   }
   ...
   if( audio_obj_ptr->audio_class_callback.callback != NULL )
   {
      audio_obj_ptr->audio_class_callback.callback( event, val, audio_obj_ptr->audio_class_callback.arg );
   }
}
...
)
{
   ...
   if( cmd > 0 )
   {
      error = USB_Class_Get_Desc( audio_obj_ptr->class_handle, cmd, audio_obj_ptr->current_interface, data );
   }
   ...
   switch( setup_packet -> request )
   {
      ...
      case GET_CUR:
         *size = 1;
         ...
         {
            error = USB_Class_Get_Desc( audio_obj_ptr->class_handle, GET_CUR_COPY_PROTECT_CONTROL, audio_obj_ptr->current_interface, data );
         }
         break;
      default:
         break;
...
)
{
   ...
   *size = 0;
   ...
   if ( cmd > 0 )
   {
      error = USB_Class_Set_Desc( audio_obj_ptr->class_handle, cmd, audio_obj_ptr->current_interface, data );
   }
   return error;
}
/**************************************************************************//*!
   ...
   *size = 0;
   switch( setup_packet -> request )
   {
      ...
      case SET_CUR:
         if( ( setup_packet->value >> 8 ) == COPY_PROTECT_CONTROL )
         {
            error = USB_Class_Set_Desc( audio_obj_ptr->class_handle, SET_CUR_COPY_PROTECT_CONTROL, audio_obj_ptr->current_interface, data );
         }
         break;
      default:
         break;
         ...
         *size = setup_packet->length;
         if ( audio_obj_ptr->mem_param_callback.callback != NULL )
         {
            error = audio_obj_ptr->mem_param_callback.callback(
                       setup_packet->request,   /* request type */
                       setup_packet->value,
                       data,                    /* pointer to the data */
                       size,                    /* size of the transfer */
                       audio_obj_ptr->mem_param_callback.arg );
         }
         break;
      default:
         break;
...
)
{
   ...
   *data = ( ( uint8_t* )setup_packet ) + USB_SETUP_PKT_SIZE;
   ...
   else if ( ( setup_packet->request_type & USB_REQUEST_CLASS_MASK ) == USB_REQUEST_CLASS_VENDOR )
   {
      ...
      {
         error = audio_obj_ptr->vendor_req_callback.callback( setup_packet, data, size, audio_obj_ptr->vendor_req_callback.arg );
      }
   }
   return error;
}
...
)
{
   ...
   devicePtr->class_handle = USB_Class_Init( devicePtr->controller_handle, USB_Class_Audio_Event, USB_Audio_Other_Requests, ( void * )devicePtr,
...
)
{
   ...
   error = _usb_device_recv_data( audio_obj_ptr->controller_handle, ep_num, buff_ptr, size );
