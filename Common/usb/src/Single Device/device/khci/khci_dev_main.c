...
/******************************************************************************
 *               BUFFER DISCRIPTOR TABLE (BDT) DESCRIPTION                    *
 ******************************************************************************/
 /**
...
//  __no_init static uint8_t bdt[512] @ "USB_BDT_Z";
    __no_init static uint8_t bdt[512];
#elif defined (__CC_ARM) || defined(__GNUC__)
    __attribute__((section(".usb_bdt"))) static uint8_t bdt[512] = { 1 };
#else
...
volatile static uint16_t tdl_index = 0;
volatile static struct tk_dne_log tdl[1024] = { 0 }; /* note, the array is for 1024 records only */
volatile static uint16_t pkb_index = 0;
volatile static uint8_t pkb[1024][8] = { 0 };
volatile static uint32_t pkbaddr[1024] = { 0 };
...
#   define KHCI_SAVE_SETUP(source) \
  { \
    pkb[pkb_index][0] = *((uint8_t *)source + 0); \
    pkb[pkb_index][1] = *((uint8_t *)source + 1); \
    pkb[pkb_index][2] = *((uint8_t *)source + 2); \
    pkb[pkb_index][3] = *((uint8_t *)source + 3); \
    pkb[pkb_index][4] = *((uint8_t *)source + 4); \
    pkb[pkb_index][5] = *((uint8_t *)source + 5); \
    pkb[pkb_index][6] = *((uint8_t *)source + 6); \
    pkb[pkb_index][7] = *((uint8_t *)source + 7); \
    pkb_index++; \
  }

#   define KHCI_SAVE_BUFADDR(addr) \
  { \
    pkbaddr[pkb_index] = (uint32_t)(addr); \
  }
...
{
    KHCI_XD_STRUCT_PTR xd_ptr_temp;

    /* prepare XD queue for RECV CONTROL ENDPOINT*/
    _usb_device_alloc_XD(state_ptr, (XD_STRUCT_PTR *)&xd_ptr_temp);
    xd_ptr_temp->G.EP_NUM = USB_CONTROL_ENDPOINT;
    xd_ptr_temp->G.BDIRECTION = USB_RECV;
    xd_ptr_temp->G.WTOTALLENGTH = ZERO_LENGTH;
    xd_ptr_temp->G.EP_TYPE = USB_CONTROL_PIPE;
    xd_ptr_temp->G.DONT_ZERO_TERMINATE = TRUE;
    xd_ptr_temp->G.WSOFAR = 0;
    xd_ptr_temp->G.WSTARTADDRESS = (uint8_t *) LONG_LE_TO_HOST(
    ...
    USB_XD_QUEUE_ENQUEUE(&state_ptr->EP_INFO[USB_CONTROL_ENDPOINT].xd_queue_recv, &xd_ptr_temp->G);
    ...
    BD_CTRL_RX(USB_CONTROL_ENDPOINT,
        state_ptr->EP_INFO[USB_CONTROL_ENDPOINT].rx_buf_odd) =
    LONG_LE_TO_HOST((uint32_t)(USB_BD_BC(
...
)
{
    ...
    BD_ADDR_RX(ep_num, state_ptr->EP_INFO[ep_num].rx_buf_odd) =
        LONG_LE_TO_HOST((uint32_t)buf_ptr);
...
)
{
    ...
    *bytes_written_ptr = buf_num_bytes;
    ...
    BD_CTRL_TX(ep_num, state_ptr->EP_INFO[ep_num].tx_buf_odd) =
        LONG_LE_TO_HOST((uint32_t)(USB_BD_BC(buf_num_bytes) |
...
)
{
    ...
    *buffer_ptr = (uint8_t *) LONG_LE_TO_HOST(BD_ADDR_TX(ep_num,
...
{
    volatile XD_STRUCT_PTR xd_ptr;
    unsigned char dir,buf_odd,token_pid;
    uint32_t len;

    USB_XD_QUEUE_GET_HEAD(&state_ptr->EP_INFO[ep_num].xd_queue_recv,&xd_ptr);

    *buffer_ptr = (uint8_t *) LONG_LE_TO_HOST(BD_ADDR_RX(ep_num,
        state_ptr->EP_INFO[ep_num].rx_buf_odd));
    ...
    token_pid = (uint8_t)((HOST_TO_LE_LONG(BD_CTRL(ep_num,dir,buf_odd)) >> 2) & 0x0000000f);
...
{
    unsigned char device_error = 0, stat, ep_num;
    volatile USB_MemMapPtr usb_ptr;
     ...
     */
    ep_num = (uint8_t)((stat & ENDPOINT_NUMBER_MASK) >> ENDPOINT_NUMBER_SHIFT);
     ...
     */
    device_error = (uint8_t)(usb_ptr->ERRSTAT & usb_ptr->ERREN);
    ...
    event.buffer_ptr = (unsigned char*)&device_error;
...
)
{
    ...
    event.buffer_ptr = (unsigned char*)NULL;
    event.len = ZERO_LENGTH;
...
)
{
    ...
    uint32_t buf_num_bytes;
    volatile USB_MemMapPtr usb_ptr;
    volatile KHCI_XD_STRUCT_PTR xd_ptr;
    ...
    { /* direction is USB_SEND*/
        ...
        USB_XD_QUEUE_GET_HEAD(&state_ptr->EP_INFO[ep_num].xd_queue_send, (XD_STRUCT_PTR *)&xd_ptr);
        /* updating the WSOFAR field */
        xd_ptr->G.WSOFAR += len;
        /* total number of bytes send are updated for upper layer */
        event.len = xd_ptr->G.WSOFAR;
         ...
         */
        buf_num_bytes = (uint32_t)(xd_ptr->G.WTOTALLENGTH - xd_ptr->G.WSOFAR);
        ...
        if((buf_num_bytes == 0)||(state_ptr->EP_INFO[ep_num].max_packet_size >
            len))
        {
            xd_ptr->G.BSTATUS = USB_STATUS_IDLE;
            /* Remove from the EP_Info send queue*/
            USB_XD_QUEUE_DEQUEUE(&state_ptr->EP_INFO[ep_num].xd_queue_send, (XD_STRUCT_PTR *)&xd_ptr);
		...
		/* According to USB spec ch5. No zero length packet is needed if has transferred
		  * exactly the amount of data specified during the Setup stage. In case a zero length
		  * packet is needed, then it shoud be sent from upper layer.
		 */
    ...
    {/* direction is USB_RECV*/
        ...
        /* Get head of the send queue */
        USB_XD_QUEUE_GET_HEAD(&state_ptr->EP_INFO[ep_num].xd_queue_recv, (XD_STRUCT_PTR *)&xd_ptr);
#if defined(KHCICFG_4BYTE_ALIGN_FIX)
        if (!xd_ptr->INTERNAL_DMA_ALIGN)
        {
            src = (uint8_t *)LONG_LE_TO_HOST(BD_ADDR_RX(ep_num, state_ptr->EP_INFO[ep_num].rx_buf_odd));
            dst = xd_ptr->G.WSTARTADDRESS + xd_ptr->G.WSOFAR;
            if(src != dst) {
                USB_mem_copy(src, dst, len);
            }
        }
#endif
        xd_ptr->G.WSOFAR += len;/* updating the WSOFAR field */
      ...
      /* dequeue if all bytes have been received or the last send transaction
         ...
         corresponding endpoint */
        if((!xd_ptr->G.WTOTALLENGTH)||(buf_num_bytes == 0)||
           (state_ptr->EP_INFO[ep_num].max_packet_size > len))
        {
            xd_ptr->G.BSTATUS = USB_STATUS_IDLE;
            /* Remove from the EP_Info send queue*/
            USB_XD_QUEUE_DEQUEUE(&state_ptr->EP_INFO[ep_num].xd_queue_recv, (XD_STRUCT_PTR *)&xd_ptr);
            /* Add it to the free queue */
            if (xd_ptr != NULL)
            _usb_device_free_XD(state_ptr, xd_ptr);
        }
...
)
{
    ...
    (void)_usb_device_set_status((_usb_device_handle)state_ptr,
        (uint8_t)USB_STATUS_DEVICE_STATE, (uint16_t)USB_STATE_SUSPEND);
...
)
{
    ...
    (void)_usb_device_set_status((_usb_device_handle)state_ptr,
        (uint8_t)USB_STATUS_DEVICE_STATE, (uint16_t)state_ptr->G.USB_DEV_STATE_B4_SUSPEND);
...
)
{
     ...
     */
    buffer_ptr = (uint8_t *)&(state_ptr->G.USB_SOF_COUNT);
    ...
    event.setup = FALSE;
    event.direction =USB_RECV;
    event.buffer_ptr = (unsigned char*)buffer_ptr;
    event.len = ZERO_LENGTH;
...
)
{
    ...
    uint16_t endp_status = USB_STATUS_IDLE;
    ...
    event.handle = (_usb_device_handle)state_ptr;
    event.setup = FALSE;
    event.direction = (uint8_t)USB_UNINITIALIZED_VAL_32;
    event.buffer_ptr = (unsigned char*)NULL;
    event.len = ZERO_LENGTH;

    /* check if the stall interrupt received was for CONTROL ENDPOINT */
    (void)_usb_device_get_status(event.handle,
        (uint8_t)(USB_CONTROL_ENDPOINT|USB_STATUS_ENDPOINT), &endp_status);
...
static void _usb_device_usbfs_isr(USB_KHCI_DEV_STATE_STRUCT_PTR state_ptr)
{
    ...
    error = (uint8_t)(usb_ptr->ISTAT & usb_ptr->INTEN);
...
)
{
    ...
    usb_dev_ptr->G.DEV_PTR = (void *) usb_ptr;
    usb_dev_ptr->G.DEV_VEC = param->VECTOR;
...
)
{
    ...
    uint8_t ep_ctrl = 0;
    KHCI_XD_STRUCT_PTR xd_temp_ptr;
    ...
    {
        _usb_device_alloc_XD (state_ptr, (XD_STRUCT_PTR *)&xd_temp_ptr);
        xd_temp_ptr->G.EP_NUM = xd_ptr->EP_NUM;
        xd_temp_ptr->G.BDIRECTION = xd_ptr->BDIRECTION;
        xd_temp_ptr->G.WTOTALLENGTH =xd_ptr->WTOTALLENGTH;
#if defined(KHCICFG_4BYTE_ALIGN_FIX)
        xd_temp_ptr->INTERNAL_DMA_ALIGN = TRUE;
#endif
        xd_temp_ptr->G.WSOFAR = xd_ptr->WSOFAR;
        xd_temp_ptr->G.EP_TYPE = xd_ptr->EP_TYPE;
        xd_temp_ptr->G.DONT_ZERO_TERMINATE = xd_ptr->DONT_ZERO_TERMINATE;
    }
     ...
     * max packet size.
     */
    max_pkt_size = (uint16_t)((xd_ptr->WMAXPACKETSIZE > MAX_EP_BUFFER_SIZE)?
    ...
    if(USB_RECV == xd_ptr->BDIRECTION)
    {
        ...
        state_ptr->EP_INFO[xd_ptr->EP_NUM].data_buffer =
            (uint8_t *)USB_mem_alloc_uncached_zero(2 * sizeof(uint8_t) * max_pkt_size);
        ...
        BD_ADDR_RX(xd_ptr->EP_NUM,
            state_ptr->EP_INFO[xd_ptr->EP_NUM].rx_buf_odd) =
                LONG_LE_TO_HOST((uint32_t)
                    state_ptr->EP_INFO[xd_ptr->EP_NUM].data_buffer);

        /* configure first start address for recv transaction */
        if((xd_ptr->WSOFAR == 0)&&(!xd_ptr->BDIRECTION))
        {
            xd_temp_ptr->G.WSTARTADDRESS = (uint8_t *) LONG_LE_TO_HOST(BD_ADDR_RX(xd_ptr->EP_NUM,
                state_ptr->EP_INFO[xd_ptr->EP_NUM].rx_buf_odd));
                /* Queue the XD taken from free list to End Point Info struncture*/
            USB_XD_QUEUE_ENQUEUE(&state_ptr->EP_INFO[xd_ptr->EP_NUM].xd_queue_recv, &xd_temp_ptr->G);
        }

        BD_CTRL_RX(xd_ptr->EP_NUM,
            state_ptr->EP_INFO[xd_ptr->EP_NUM].rx_buf_odd) =
                LONG_LE_TO_HOST((uint32_t)(USB_BD_BC(max_pkt_size) | control[0]));
        ...
        BD_CTRL_RX(xd_ptr->EP_NUM,
            (state_ptr->EP_INFO[xd_ptr->EP_NUM].rx_buf_odd^1)) =
                LONG_LE_TO_HOST((uint32_t)(USB_BD_BC(max_pkt_size) | control[1]));
    }
    else /* Set the BDT and buffer data pointer for Send direction */
    {
        ...
        BD_CTRL_TX(xd_ptr->EP_NUM,
            state_ptr->EP_INFO[xd_ptr->EP_NUM].tx_buf_odd) =
                LONG_LE_TO_HOST((uint32_t)(USB_BD_BC(max_pkt_size) |
...
)
{
    ...
    BD_ADDR(ep_num,direction,EVEN_BUFF) =
        LONG_LE_TO_HOST_CONST((uint32_t)USB_UNINITIALIZED_VAL_32);
...
)
{
    ...
    uint8_t *buf_ptr;
    volatile USB_KHCI_DEV_STATE_STRUCT_PTR state_ptr = (USB_KHCI_DEV_STATE_STRUCT_PTR)handle;
    KHCI_XD_STRUCT_PTR xd_temp_ptr = (KHCI_XD_STRUCT_PTR)xd_ptr;
...
#if defined(KHCICFG_4BYTE_ALIGN_FIX)
    if (buf_num_bytes & USB_DMA_ALIGN_MASK || (_mem_size)buf_ptr & USB_DMA_ALIGN_MASK)
    {
        xd_temp_ptr->INTERNAL_DMA_ALIGN = FALSE;
...
)
{
    ...
    {/* USB_SEND */
        BD_CTRL_TX(ep_num, state_ptr->EP_INFO[ep_num].tx_buf_odd) =
            LONG_LE_TO_HOST((uint32_t)(USB_BD_BC(state_ptr->EP_INFO[ep_num].
...
)
{
    ...
    {/* USB_SEND */
        ...
        BD_CTRL_TX(ep_num, state_ptr->EP_INFO[ep_num].tx_buf_odd) =
            LONG_LE_TO_HOST((uint32_t)(USB_BD_BC(state_ptr->EP_INFO[ep_num].max_packet_size) |
    ...
    {/* USB_RECV */
        ...
        /* prepare XD queue for RECV CONTROL ENDPOINT*/
        _usb_device_alloc_XD (state_ptr, (XD_STRUCT_PTR *)&xd_ptr_temp);
        xd_ptr_temp->G.EP_NUM = ep_num;
        xd_ptr_temp->G.BDIRECTION = USB_RECV;
        xd_ptr_temp->G.WTOTALLENGTH = ZERO_LENGTH;
        xd_ptr_temp->G.EP_TYPE = state_ptr->EP_INFO[ep_num].type;
        xd_ptr_temp->G.DONT_ZERO_TERMINATE = TRUE;
#if defined(KHCICFG_4BYTE_ALIGN_FIX)
        xd_ptr_temp->INTERNAL_DMA_ALIGN = TRUE;
#endif
        ...
        USB_XD_QUEUE_ENQUEUE(&state_ptr->EP_INFO[ep_num].xd_queue_recv, &xd_ptr_temp->G);
...
)
{
    ...
    (void)_usb_device_set_status(handle, (uint8_t)USB_STATUS_DEVICE_STATE,
        (uint16_t)USB_STATE_UNKNOWN);
...
)
{
    ...
    *((uint16_t*)&buffer_ptr[2]) = SHORT_LE_TO_HOST(*((uint16_t*)&buffer_ptr[2]));
    *((uint16_t*)&buffer_ptr[4]) = SHORT_LE_TO_HOST(*((uint16_t*)&buffer_ptr[4]));
    *((uint16_t*)&buffer_ptr[6]) = SHORT_LE_TO_HOST(*((uint16_t*)&buffer_ptr[6]));
...
)
{
    ...
    if(ep_num < USB_MAX_ENDPOINTS)
    {
        uint8_t ep_control = usb_ptr->ENDPOINT[ep_num].ENDPT;
