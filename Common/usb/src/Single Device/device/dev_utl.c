...
)
{
   ...
   if ( ( ( USB_DEV_CALLBACK_FUNCTIONS_STRUCT_PTR ) usb_dev_ptr->CALLBACK_STRUCT_PTR )->DEV_UNSTALL_ENDPOINT != NULL )
   {
      error = ( ( USB_DEV_CALLBACK_FUNCTIONS_STRUCT_PTR ) usb_dev_ptr->CALLBACK_STRUCT_PTR )->DEV_UNSTALL_ENDPOINT( handle, ep_num, direction );
   }
   else
   {
      return USBERR_ERROR;
...
(
   ...
   uint8_t              component,  /* [IN] What to get the error of */
   uint16_t             *error      /* [OUT] The requested error */
)
{
   /* Body */
   volatile USB_DEV_STATE_STRUCT_PTR usb_dev_ptr;
   ...
   switch ( component )
   {
      ...
      default:
         ...
         {
            if ( ( ( USB_DEV_CALLBACK_FUNCTIONS_STRUCT_PTR ) usb_dev_ptr->CALLBACK_STRUCT_PTR )->DEV_GET_ENDPOINT_STATUS != NULL )
            {
               ( ( USB_DEV_CALLBACK_FUNCTIONS_STRUCT_PTR ) usb_dev_ptr->CALLBACK_STRUCT_PTR )->DEV_GET_ENDPOINT_STATUS( handle,
                                                                                                                       ( uint8_t )( component & USB_STATUS_ENDPOINT_NUMBER_MASK ),
                                                                                                                       error );
            }
            else
            {
...
(
   _usb_device_handle   handle,     /* [IN] Handle to the usb device */
   uint8_t              component,  /* [IN] What to set the error of */
   uint16_t             setting     /* [IN] What to set the error to */
)
{
   volatile USB_DEV_STATE_STRUCT_PTR usb_dev_ptr;
   ...
   switch ( component )
   {
      ...
      case USB_STATUS_ADDRESS:
         ...
         {
            error = ( ( USB_DEV_CALLBACK_FUNCTIONS_STRUCT_PTR ) usb_dev_ptr->CALLBACK_STRUCT_PTR )->DEV_SET_ADDRESS( ( void * )usb_dev_ptr, ( uint8_t )( setting & 0x00FF ) );
         }
         else
         {
#if _DEBUG
      ...
      default:
         if ( component & USB_STATUS_ENDPOINT )
         {
            if ( ( ( USB_DEV_CALLBACK_FUNCTIONS_STRUCT_PTR ) usb_dev_ptr->CALLBACK_STRUCT_PTR )->DEV_SET_ENDPOINT_STATUS != NULL )
            {
               error = ( ( USB_DEV_CALLBACK_FUNCTIONS_STRUCT_PTR ) usb_dev_ptr->CALLBACK_STRUCT_PTR )->DEV_SET_ENDPOINT_STATUS( handle, component, setting );
            }
            else
            {
#if _DEBUG
...
)
{
   ...
   if ( ( ( USB_DEV_CALLBACK_FUNCTIONS_STRUCT_PTR ) usb_dev_ptr->CALLBACK_STRUCT_PTR )->DEV_STALL_ENDPOINT != NULL )
   {
      error = ( ( USB_DEV_CALLBACK_FUNCTIONS_STRUCT_PTR ) usb_dev_ptr->CALLBACK_STRUCT_PTR )->DEV_STALL_ENDPOINT( handle, ep_num, direction );
   }
   else
   {
#if _DEBUG
...
)
{
   ...
   if ( ( ( USB_DEV_CALLBACK_FUNCTIONS_STRUCT_PTR ) usb_dev_ptr->CALLBACK_STRUCT_PTR )->DEV_ASSERT_RESUME != NULL )
   {
      error = ( ( USB_DEV_CALLBACK_FUNCTIONS_STRUCT_PTR ) usb_dev_ptr->CALLBACK_STRUCT_PTR )->DEV_ASSERT_RESUME( handle );
   }
   else
   {
#if _DEBUG
