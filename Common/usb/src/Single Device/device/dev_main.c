...
(
   ...
   void                 *xd_ptr  /* [IN] the dTD to enqueue */
)
{
   /* Body */
...
(
   struct usb_dev_if_struct   *usb_if,    /* [IN] the USB device controller to initialize */
   _usb_device_handle         *handle,    /* [OUT] the USB_USB_dev_initialize state structure */
   _mqx_uint                  endpoints   /* [IN] number of endpoints to initialize */
)
{
   /* Body */
   ...
   usb_dev_ptr->CALLBACK_STRUCT_PTR = ( void * ) usb_c[i].DEV_IF;
   ...
   if ( ( ( USB_DEV_CALLBACK_FUNCTIONS_STRUCT_PTR ) usb_dev_ptr->CALLBACK_STRUCT_PTR )->DEV_INIT != NULL )
   {
...
)
{
   ...
   USB_mem_free( ( void * )usb_dev_ptr->XD_BASE );

   /* Free all XD scratch memory */
   USB_mem_free( ( void * )usb_dev_ptr->XD_SCRATCH_STRUCT_BASE );

   /* Free the temp ep init XD */
   USB_mem_free( ( void * )usb_dev_ptr->TEMP_XD_PTR );
...
(
   ...
   USB_EP_STRUCT_PTR    ep_ptr,  /* [IN] the endpoint structure, include members such as endpoint number, endpoint type, endpoint direction and the max packet size */
   uint8_t              flag     /* [IN] After all data is transfered, should we terminate the transfer with a zero length packet if the last packet size == MAX_PACKET_SIZE?  */
)
{
   /* Body */
   USB_STATUS                    error = 0;
   ...
   if ( ( ( USB_DEV_CALLBACK_FUNCTIONS_STRUCT_PTR ) usb_dev_ptr->CALLBACK_STRUCT_PTR )->DEV_INIT_ENDPOINT != NULL )
   {
      error = ( ( USB_DEV_CALLBACK_FUNCTIONS_STRUCT_PTR ) usb_dev_ptr->CALLBACK_STRUCT_PTR )->DEV_INIT_ENDPOINT( handle, usb_dev_ptr->TEMP_XD_PTR );
   }
   else
   {
#ifdef _DEV_DEBUG
...
(
   _usb_device_handle         handle,                          /* [IN] Handle to the USB device */
   uint8_t                    type,                            /* [IN] type of event or endpoint number to service */
   void( _CODE_PTR_ service )( PTR_USB_EVENT_STRUCT, void * ), /* [IN] Pointer to the service's callback function */
   void                       *arg                             /*[IN] User Argument to be passed to Services when invoked.*/
)
{
   /* Body */
   USB_DEV_STATE_STRUCT_PTR   usb_dev_ptr;
...
(
   ...
   PTR_USB_EVENT_STRUCT event /* [IN] pointer to event structure  */
)
{
   /* Body */
   USB_DEV_STATE_STRUCT_PTR      usb_dev_ptr;
...
(
   _usb_device_handle   handle,     /* [IN] the USB_USB_dev_initialize state structure */
   uint8_t              ep_num,     /* [IN] direction */
   uint8_t              direction   /* [IN] the Endpoint number */
)
{
   /* Body */
   USB_STATUS                    error;
   ...
   if ( ( ( USB_DEV_CALLBACK_FUNCTIONS_STRUCT_PTR ) usb_dev_ptr->CALLBACK_STRUCT_PTR )->DEV_GET_TRANSFER_STATUS != NULL )
   {
      error = ( ( USB_DEV_CALLBACK_FUNCTIONS_STRUCT_PTR ) usb_dev_ptr->CALLBACK_STRUCT_PTR )->DEV_GET_TRANSFER_STATUS( handle, ep_num, direction );
   }
   else
   {
#ifdef _DEV_DEBUG
...
(
   ...
   uint8_t              *buff_ptr   /* [IN] buffer for receiving Setup packet */
)
{
   /* Body */
   USB_DEV_STATE_STRUCT_PTR      usb_dev_ptr;
   ...
   if ( ( ( USB_DEV_CALLBACK_FUNCTIONS_STRUCT_PTR ) usb_dev_ptr->CALLBACK_STRUCT_PTR )->DEV_GET_SETUP_DATA != NULL )
   {
      error = ( ( USB_DEV_CALLBACK_FUNCTIONS_STRUCT_PTR ) usb_dev_ptr->CALLBACK_STRUCT_PTR )->DEV_GET_SETUP_DATA( handle, ep_num, buff_ptr );
   }
   else
   {
#ifdef _DEV_DEBUG
...
(
   _usb_device_handle   handle,  /* [IN] the USB_USB_dev_initialize state structure */
   uint8_t              address  /* [IN] the USB address to be set in the hardware */
)
{
   USB_DEV_STATE_STRUCT_PTR      usb_dev_ptr;
   USB_STATUS                    error;
   ...
   if ( ( ( USB_DEV_CALLBACK_FUNCTIONS_STRUCT_PTR ) usb_dev_ptr->CALLBACK_STRUCT_PTR )->DEV_SET_ADDRESS != NULL )
   {
      error = ( ( USB_DEV_CALLBACK_FUNCTIONS_STRUCT_PTR ) usb_dev_ptr->CALLBACK_STRUCT_PTR )->DEV_SET_ADDRESS( handle, address );
   }
   else
   {
#ifdef _DEV_DEBUG
