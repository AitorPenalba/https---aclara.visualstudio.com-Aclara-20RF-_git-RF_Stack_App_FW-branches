 ...
 *****************************************************************************/
static uint8_t USB_PHDC_Other_Requests( USB_SETUP_STRUCT * setup_packet, uint8_t **data, uint32_t *size, void   *arg ); 
static void USB_Class_PHDC_Event( uint8_t event, void * value, void   *arg );
/*****************************************************************************
   Local Functions
 *****************************************************************************/
/*************************************************************************//*!
...
*****************************************************************************/
static int32_t USB_Phdc_Free_Handle( PHDC_HANDLE handle )
{
...
******************************************************************************
   ...
   received.
*****************************************************************************/
static void USB_Class_PHDC_Event( uint8_t event, void* val, void *arg )
{
   uint8_t     ep_count;
   ...
   if( event == USB_APP_ENUM_COMPLETE )
   {
      ...
      {
         uint8_t component = ( uint8_t )( phdc_obj_ptr->ep_data.ep_rx[ep_count].endpoint | ( USB_RECV << COMPONENT_PREPARE_SHIFT ) );
         ...
         /* initialize endpoint */
         ( void )_usb_device_init_endpoint( phdc_obj_ptr->controller_handle, &ep_struct, TRUE );
         ...
         ( void )_usb_device_set_status( phdc_obj_ptr->controller_handle, ( uint8_t )( USB_STATUS_ENDPOINT | component ), ( uint16_t )USB_STATUS_IDLE );
      ...
      {
         uint8_t component = ( uint8_t )( phdc_obj_ptr->ep_data.ep_tx[ep_count].endpoint | ( USB_SEND << COMPONENT_PREPARE_SHIFT ) );
   ...
   {
      phdc_obj_ptr->phdc_callback.callback( event, val, phdc_obj_ptr->phdc_callback.arg );
   }
}
 ...
 *****************************************************************************/
static uint8_t USB_PHDC_Other_Requests
(
      ...
      *size = 0;
      ...
      switch( setup_packet->request )
      {
         ...
         case CLEAR_FEATURE_REQUEST:
         {
            ...
            phdc_obj_ptr->phdc_metadata = ( bool )( ( setup_packet->request &
                                                    USB_SET_REQUEST_MASK ) >> 1 );
            /* inform the application that feature has changed */
            if ( phdc_obj_ptr->phdc_callback.callback != NULL ) phdc_obj_ptr->phdc_callback.callback( USB_APP_FEATURE_CHANGED,
         ...
         case GET_STATUS_REQUEST:
         {
            ...
            *data = ( uint8_t * )( &phdc_obj_ptr->phdc_ep_has_data );
            *size = setup_packet->length;
            break;
         }
      }
   }
   else if( ( setup_packet->request_type & USB_REQUEST_CLASS_MASK ) == USB_REQUEST_CLASS_VENDOR )
   {
      /* vendor specific request  */
      if( phdc_obj_ptr->vendor_callback.callback != NULL )
      {
         error = phdc_obj_ptr->vendor_callback.callback( setup_packet, data, size, phdc_obj_ptr->vendor_callback.arg );
      }
   }
   return error;
}
...
void USB_PHDC_Endpoint_Service( PTR_USB_EVENT_STRUCT event, void *arg )
{
   ...
   uint8_t                       meta_data_packet;
   uint8_t                       transfers_left;
#endif
   ...
   if( event->direction == USB_SEND )
   {
      ...
      if( consumer != producer )
      {
         ...
         if( qos_bin.meta_data == TRUE )
         {
            ...
            phdc_obj_ptr->ep_data.ep_tx[index].transfers_left = qos_bin.num_tfr;
            /* initialize current_qos of the transfers that follow the
               meta data packet */
            phdc_obj_ptr->ep_data.ep_tx[index].current_qos = qos_bin.qos;
         }
#endif
         /* send data */
         ( void )USB_Class_Send_Data( phdc_obj_ptr->class_handle, qos_bin.channel, qos_bin.app_buff, qos_bin.size );
      }
      else /* bin is empty */
      {
      ...
      phdc_obj_ptr->phdc_callback.callback( USB_APP_SEND_COMPLETE, ( void* )( &event_send_complete ), phdc_obj_ptr->phdc_callback.arg );
   }
   else /* direction is USB_RECV */
   {
      ...
      if( phdc_obj_ptr->ep_data.ep_rx[index].buff_ptr == NULL )
      {
         ...
         phdc_obj_ptr->phdc_callback.callback( USB_APP_GET_DATA_BUFF, ( void* )( &rx_buff ), phdc_obj_ptr->phdc_callback.arg );
         ...
         {
            /* Received Complete packet first time itself */
            USB_mem_copy( event->buffer_ptr, rx_buff.out_buff, rx_buff.out_size );
      ...
      if( event->len == phdc_obj_ptr->ep_data.ep_rx[index].buffer_size )
      {
         ...
         meta_data_packet = ( uint8_t )strncmp( phdc_obj_ptr->meta_data_msg_preamble.signature, msg_preamble_signature, METADATA_PREAMBLE_SIGNATURE );
         ...
         if( ( phdc_obj_ptr->phdc_metadata == FALSE ) || ( transfers_left != 0 &&
               meta_data_packet != 0 ) )
         {
            phdc_obj_ptr->ep_data.ep_rx[index].transfers_left = ( uint8_t )( transfers_left - 1 );
            ...
            phdc_obj_ptr->phdc_callback.callback( USB_APP_DATA_RECEIVED, ( void* )( &event_data_recieved ), phdc_obj_ptr->phdc_callback.arg );
         }
         else if( ( transfers_left == 0 ) && ( meta_data_packet == 0 ) )
         {
            ...
            phdc_obj_ptr->ep_data.ep_rx[index].transfers_left = phdc_obj_ptr->meta_data_msg_preamble.num_tfr;
            phdc_obj_ptr->ep_data.ep_rx[index].current_qos = phdc_obj_ptr->meta_data_msg_preamble.qos;
            /* set the event phdc meta data params structure*/
            metadata_params.channel = event->ep_num;
            metadata_params.num_tfr = phdc_obj_ptr->meta_data_msg_preamble.num_tfr;
            metadata_params.qos = phdc_obj_ptr->meta_data_msg_preamble.qos;
            metadata_params.metadata_ptr = &phdc_obj_ptr->meta_data_msg_preamble.opaque_data[0];

            /*notify the appl that meta data params has changed*/
            phdc_obj_ptr->phdc_callback.callback( USB_APP_META_DATA_PARAMS_CHANGED, ( void* )( &metadata_params ), phdc_obj_ptr->phdc_callback.arg );
         }
#else
         /* set the data received structure */
         event_data_recieved.qos = phdc_obj_ptr->ep_data.ep_rx[index].qos;
...
)
{
   ...
   if( ( uint8_t )( producer - consumer ) != ( uint8_t )( MAX_QOS_BIN_ELEMS ) )
   {
      ...
      phdc_obj_ptr->ep_data.ep_tx[tx_index].qos_bin[queue_num].channel = channel;
      phdc_obj_ptr->ep_data.ep_tx[tx_index].qos_bin[queue_num].meta_data = meta_data;
      phdc_obj_ptr->ep_data.ep_tx[tx_index].qos_bin[queue_num].num_tfr = num_tfr;
      phdc_obj_ptr->ep_data.ep_tx[tx_index].qos_bin[queue_num].qos = qos;
      phdc_obj_ptr->ep_data.ep_tx[tx_index].qos_bin[queue_num].app_buff = app_buff;
      ...
      if( ( uint8_t )( producer - consumer ) == ( uint8_t )1 )
      {
               ...
               ( phdc_obj_ptr->ep_data.ep_tx[tx_index].transfers_left != 0 ) )
         {
            ...
            phdc_obj_ptr->phdc_callback.callback( USB_APP_ERROR, NULL, phdc_obj_ptr->phdc_callback.arg );
            return USBERR_TX_FAILED;
         }
         else if( meta_data == TRUE )
         {
            /* initialize transfers left if the packet is a meta data */
            phdc_obj_ptr->ep_data.ep_tx[tx_index].transfers_left = num_tfr;
            /* initialize current_qos of the transfers that follow the
               meta data packet */
         ...
         /* send the packet if there is no packet (other than this packet)
            in the queue */
         error = USB_Class_Send_Data( phdc_obj_ptr->class_handle, channel, app_buff, size );
      }
   }
   else /* bin is full */
   {
...
/**************************************************************************//*!
   ...
   @return PHDC_HANDLE      : When Successful
           Others           : Errors
 ******************************************************************************
...
)
{
   ...
   error = _usb_device_init( USBCFG_DEFAULT_DEVICE_CONTROLLER, ( _usb_device_handle * )&devicePtr->controller_handle, ( uint8_t )( ( info->count ) + 1 ) );
   if( error != USB_OK )
   {
      goto error1;
   }
   ...
   devicePtr->class_handle = USB_Class_Init( devicePtr->controller_handle, USB_Class_PHDC_Event, USB_PHDC_Other_Requests, ( void * )devicePtr, phdc_config_ptr->desc_callback_ptr );

   /* initialize endpoint data structure for all endpoints */
   for( ep_count = 0; ep_count < info->count; ep_count++ )
   {
      if( ( info->ep[ep_count].direction == USB_RECV ) & ( devicePtr->ep_data.count_rx < PHDC_RX_ENDPOINTS ) )
      {
         count = devicePtr->ep_data.count_rx;

         /* initialize endpoint data structure for recv endpoint */
         devicePtr->ep_data.ep_rx[count].endpoint = info->ep[ep_count].ep_num;
         devicePtr->ep_data.ep_rx[count].type = info->ep[ep_count].type;
         devicePtr->ep_data.ep_rx[count].size = info->ep[ep_count].size;
         devicePtr->ep_data.ep_rx[count].qos = ( info->ep[ep_count].type == USB_INTERRUPT_PIPE ? PHDC_INT_IN_QOS : PHDC_BULK_IN_QOS );
         devicePtr->ep_data.ep_rx[count].current_qos = INVALID_VAL;
         /* register endpoint service on the endpoint*/
         ( void )_usb_device_register_service( devicePtr->controller_handle, devicePtr->ep_data.ep_rx[count].endpoint, USB_PHDC_Endpoint_Service, ( void * )devicePtr );
         /* increment count_rx by 1 */
         devicePtr->ep_data.count_rx++;
      }
      else if( ( info->ep[ep_count].direction == USB_SEND ) & ( devicePtr->ep_data.count_tx < PHDC_TX_ENDPOINTS ) )
      {
         count = devicePtr->ep_data.count_tx;
         /* initialize endpoint data structure for send endpoint */
         devicePtr->ep_data.ep_tx[count].endpoint = ( uint8_t )info->ep[ep_count].ep_num;
         devicePtr->ep_data.ep_tx[count].type = ( uint8_t )info->ep[ep_count].type;
         devicePtr->ep_data.ep_tx[count].size = ( uint32_t )info->ep[ep_count].size;
         devicePtr->ep_data.ep_tx[count].qos = ( info->ep[ep_count].type == USB_INTERRUPT_PIPE ? PHDC_INT_OUT_QOS : PHDC_BULK_OUT_QOS );
         devicePtr->ep_data.ep_tx[count].current_qos = ( uint8_t )INVALID_VAL;
         devicePtr->ep_data.ep_tx[count].bin_consumer = ( uint8_t )0x00;
         devicePtr->ep_data.ep_tx[count].bin_producer = ( uint8_t )0x00;
         /* register endpoint service on the endpoint*/
   ...
   ( void )_usb_device_set_status( devicePtr->controller_handle, ( uint8_t )USB_STATUS_DEVICE_STATE, ( uint16_t )USB_STATE_POWERED );
...
)
{
   ...
   status = _usb_device_recv_data( phdc_obj_ptr->controller_handle, phdc_obj_ptr->ep_data.ep_rx[rx_index].endpoint, buff_ptr, size );
