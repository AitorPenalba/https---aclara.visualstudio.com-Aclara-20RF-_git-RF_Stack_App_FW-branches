...
typedef uint8_t ( _CODE_PTR_ USB_REQ_FUNC )( USB_SETUP_STRUCT *, uint8_t **, uint32_t*, void   *arg );

/*callback function pointer structure for application to provide class params*/
typedef uint8_t ( _CODE_PTR_ USB_CLASS_SPECIFIC_HANDLER_FUNC )( uint8_t, uint16_t, uint8_t **, uint32_t*, void   *arg );
...
typedef struct usb_class_specific_handler_callback_struct
{
   ...
   void   *arg;
} USB_CLASS_SPECIFIC_HANDLER_CALLBACK_STRUCT, *USB_CLASS_SPECIFIC_HANDLER_CALLBACK_STRUCT_PTR;
...
{
   uint32_t handle;
   uint8_t ( _CODE_PTR_ GET_DESC )( uint32_t handle, uint8_t type, uint8_t str_num, uint16_t index, uint8_t **descriptor, uint32_t *size );
   USB_ENDPOINTS * ( _CODE_PTR_ GET_DESC_ENDPOINTS )( uint32_t handle );
   uint8_t ( _CODE_PTR_ GET_DESC_INTERFACE )( uint32_t handle, uint8_t interface, uint8_t *alt_interface );
   uint8_t ( _CODE_PTR_ SET_DESC_INTERFACE )( uint32_t handle, uint8_t interface, uint8_t alt_interface );
   bool ( _CODE_PTR_ IS_DESC_VALID_CONFIGURATION )( uint32_t handle, uint16_t config_val );
   bool ( _CODE_PTR_ DESC_REMOTE_WAKEUP )( uint32_t handle );
   uint8_t ( _CODE_PTR_ DESC_SET_FEATURE )( uint32_t handle, int32_t cmd, uint8_t in_data, uint8_t **feature );
   uint8_t ( _CODE_PTR_ DESC_GET_FEATURE )( uint32_t handle, int32_t cmd, uint8_t in_data, uint8_t ** feature );
