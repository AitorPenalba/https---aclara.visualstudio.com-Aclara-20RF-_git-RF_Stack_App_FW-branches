...
//        USB_ENDPOINTS *usb_ep_data = cdc_obj_ptr->usb_ep_data;
   ...
   if( consumer != producer )
   {
      ...
      ( void )USB_Class_Send_Data( cdc_obj_ptr->class_handle, queue.channel, queue.app_data.data_ptr, queue.app_data.data_size );
   }
#endif

   if( cdc_obj_ptr->cdc_class_cb.callback != NULL )
   {
      uint8_t event_type = USB_APP_SEND_COMPLETE;
      cdc_obj_ptr->cdc_class_cb.callback( event_type, NULL, cdc_obj_ptr->cdc_class_cb.arg );
   }
}
...
//        USB_ENDPOINTS *usb_ep_data = cdc_obj_ptr->usb_ep_data;
   ...
   if( cdc_obj_ptr->param_callback.callback != NULL )
   {
      ...
      cdc_obj_ptr->param_callback.callback( event_type, ( void* )&bulk_in_recv, cdc_obj_ptr->param_callback.arg );
   }
}
...
void USB_Service_Dic_Bulk_Out( PTR_USB_EVENT_STRUCT event, void *arg )
{
   ...
   if( cdc_obj_ptr->param_callback.callback != NULL )
   {
      cdc_obj_ptr->param_callback.callback( event_type, ( void* )&bulk_out_recv, cdc_obj_ptr->param_callback.arg );
   }
}
...
void USB_Class_CDC_Event( uint8_t event, void* val, void *arg )
{
   ...
   if( event == USB_APP_ENUM_COMPLETE )
   {
      ...
      while( count < usb_ep_data->count )
      {
         ...
         ( void )_usb_device_init_endpoint( cdc_obj_ptr->controller_handle, ep_struct_ptr, TRUE );
   ...
   if( cdc_obj_ptr->cdc_class_cb.callback != NULL )
   {
      cdc_obj_ptr->cdc_class_cb.callback( event, val, cdc_obj_ptr->cdc_class_cb.arg );
   }
}
...
)
{
...
#if RNDIS_SUPPORT
   USB_mem_copy( cdc_config_ptr->mac_address, devicePtr->mac_address, sizeof( _enet_address ) );
   ...
   devicePtr->class_handle = USB_Class_Init( devicePtr->controller_handle, USB_Class_CDC_Event, USB_CDC_Other_Requests, ( void * )devicePtr, cdc_config_ptr->desc_callback_ptr );
...
)
{
   ...
   status = _usb_device_recv_data( cdc_obj_ptr->controller_handle, ep_num, buff_ptr, size );
