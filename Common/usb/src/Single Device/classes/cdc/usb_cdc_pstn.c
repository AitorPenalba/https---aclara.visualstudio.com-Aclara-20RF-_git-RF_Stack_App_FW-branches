...
static unsigned char g_vendor_info[VENDOR_INFO_SIZE] = " FSL RNDIS PORT ";
#endif
...
)
{
   ...
   USB_mem_copy( g_serial_state_buf, cdc_pstn_ptr->serial_state_buf, ( NOTIF_PACKET_SIZE + UART_BITMAP_SIZE ) );
...
)
{
   ...
   *size = 0;
   ...
   cdc_pstn_ptr->current_interface =  ( uint8_t )setup_packet->index ;
   error = USB_Class_Set_Desc( cdc_obj_ptr->class_handle, SET_LINE_CODING, cdc_pstn_ptr->current_interface, ( uint8_t ** )&setup_packet );
 ...
 *****************************************************************************/
uint8_t PSTN_Set_Ctrl_Line_State
(
   CDC_DEVICE_STRUCT_PTR   cdc_obj_ptr,
   USB_SETUP_STRUCT        *setup_packet,
   uint8_t                 **data,
...
void PSTN_Send_Serial_State( CDC_DEVICE_STRUCT_PTR cdc_obj_ptr )
{
   ...
   *( uint16_t* )&cdc_pstn_ptr->serial_state_buf[NOTIF_PACKET_SIZE + UART_BITMAP_SIZE - 2] =
   ...
   ( void )USB_Class_CDC_Send_Data( cdc_obj_ptr->cdc_handle, cdc_obj_ptr->cic_send_endpoint, cdc_pstn_ptr->serial_state_buf, val );
}
...
void PSTN_Response_Available( CDC_DEVICE_STRUCT_PTR cdc_obj_ptr )
{
   ...
   ( void )USB_Class_CDC_Send_Data( cdc_obj_ptr->cdc_handle, cdc_obj_ptr->cic_send_endpoint, g_response_available_data, NOTIF_PACKET_SIZE );
}
...
)
{
   ...
   USB_mem_copy( setup_packet + 1, cdc_pstn_ptr->rndis_command_ptr, setup_packet->length );
...
)
{
   ...
   *( ( uint32_t * )cdc_pstn_ptr->response_data_ptr + 1 ) = LONG_LE_TO_HOST_CONST( RESPONSE_RNDIS_INITIALIZE_MSG_SIZE );
   ...
   USB_mem_copy( ( ( uint32_t * )( cdc_pstn_ptr->rndis_command_ptr ) + 2 ), ( ( uint32_t * )cdc_pstn_ptr->response_data_ptr + 2 ), sizeof( uint32_t ) );
   ...
   USB_mem_copy( ( ( uint32_t * )( cdc_pstn_ptr->rndis_command_ptr ) + 3 ), ( ( uint32_t * )cdc_pstn_ptr->response_data_ptr + 4 ), sizeof( uint32_t ) );

   /* preparing for Byte 20-23 : MinorVersion*/
   USB_mem_copy( ( ( uint32_t * )( cdc_pstn_ptr->rndis_command_ptr ) + 4 ), ( ( uint32_t * )cdc_pstn_ptr->response_data_ptr + 5 ), sizeof( uint32_t ) );
   ...
   *( ( uint32_t * )cdc_pstn_ptr->response_data_ptr + 8 ) = LONG_LE_TO_HOST_CONST( RNDIS_SINGLE_PACKET_TRANSFER );
   ...
   USB_mem_copy( &max_tx_size, ( ( uint32_t * )cdc_pstn_ptr->response_data_ptr + 9 ), sizeof( uint32_t ) );
   ...
   *( ( uint32_t * )cdc_pstn_ptr->response_data_ptr + 5 ) = LONG_LE_TO_HOST_CONST( 0x00000010 );
   ...
   switch( *oid_ptr )
   {
      case OID_GEN_SUPPORTED_LIST :
         ...
         USB_mem_copy( g_list_supp_oid, ( uint32_t * )cdc_pstn_ptr->response_data_ptr + 6, info_buf_len );
         break;
      ...
      case OID_GEN_MAXIMUM_FRAME_SIZE :
         ...
         info_buf_len = sizeof( uint32_t );
         *( ( uint32_t * )cdc_pstn_ptr->response_data_ptr + 6 ) = LONG_LE_TO_HOST( cdc_obj_ptr->rndis_max_frame_size );
         break;
      case OID_GEN_LINK_SPEED :
         ...
         if( cdc_pstn_ptr->pstn_callback.callback != NULL )
         {
            cdc_pstn_ptr->pstn_callback.callback( USB_APP_GET_LINK_SPEED, ( void* )( ( uint32_t * )cdc_pstn_ptr->response_data_ptr + 6 ), cdc_pstn_ptr->pstn_callback.arg );
         }

         *( ( uint32_t * )cdc_pstn_ptr->response_data_ptr + 6 ) = LONG_LE_TO_HOST( *( ( uint32_t * )cdc_pstn_ptr->response_data_ptr + 6 ) );
         break;
      case OID_GEN_TRANSMIT_BLOCK_SIZE :
         ...
         info_buf_len = sizeof( uint32_t );
         *( ( uint32_t * )cdc_pstn_ptr->response_data_ptr + 6 ) = LONG_LE_TO_HOST( ( uint32_t )cdc_obj_ptr->dic_send_pkt_size );
         break;
      case OID_GEN_RECEIVE_BLOCK_SIZE :
         ...
         info_buf_len = sizeof( uint32_t );
         *( ( uint32_t * )cdc_pstn_ptr->response_data_ptr + 6 ) = LONG_LE_TO_HOST( ( uint32_t )cdc_obj_ptr->dic_recv_pkt_size );
         break;
      ...
      case OID_802_3_PERMANENT_ADDRESS :
         ...
         USB_mem_copy( cdc_obj_ptr->mac_address, ( uint8_t * )( ( uint32_t * )cdc_pstn_ptr->response_data_ptr + 6 ), info_buf_len );
   ...
   switch( *oid_ptr )
   {
      case OID_GEN_CURRENT_PACKET_FILTER :
         ...
         else
         {
            ...
            if( cdc_pstn_ptr->pstn_callback.callback != NULL )
            {
               cdc_pstn_ptr->pstn_callback.callback( USB_APP_GET_LINK_STATUS, ( void* )( &media_connected ), cdc_pstn_ptr->pstn_callback.arg );
            }
