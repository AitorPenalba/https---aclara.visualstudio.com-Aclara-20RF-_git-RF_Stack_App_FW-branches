      ...
      *size = 0;
      ...
      switch( setup_packet->request )
      {
         ...
         case CLEAR_FEATURE_REQUEST:
         {
            ...
            if ( phdc_obj_ptr->phdc_callback.callback != NULL )
               phdc_obj_ptr->phdc_callback.callback( USB_APP_FEATURE_CHANGED, ( void* )( &phdc_obj_ptr->phdc_metadata ), phdc_obj_ptr->phdc_callback.arg );
#endif
            break;
         }
   ...
   else /* direction is USB_RECV */
   {
      ...
      if( phdc_obj_ptr->ep_data.ep_rx[index].buff_ptr == NULL )
      {
         ...
         else
         {
            ...
            ( void )USB_Class_PHDC_Recv_Data( phdc_obj_ptr->phdc_handle, phdc_obj_ptr->ep_data.ep_rx[index].qos, rx_buff.out_buff, phdc_obj_ptr->ep_data.ep_rx[index].buffer_size );
            return;
         }
      }
