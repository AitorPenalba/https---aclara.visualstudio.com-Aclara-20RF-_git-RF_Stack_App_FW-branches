...
)
{
   ...
   USB_mem_copy( desc_callback_ptr, &usb_fw_ptr->desc, sizeof( DESC_CALLBACK_FUNCTIONS_STRUCT ) );
...
void USB_Control_Service ( PTR_USB_EVENT_STRUCT event, void *arg )
{
   ...
   ( void )_usb_device_get_status( event->handle, ( uint8_t )USB_STATUS_DEVICE_STATE, &device_state );

   if ( event->setup == TRUE )
   {
      USB_mem_copy( event->buffer_ptr, &usb_fw_ptr->setup_packet, USB_SETUP_PKT_SIZE );
            ...
            USB_REQUEST_CLASS_STRD )
      {
         ...
         if ( g_standard_request[usb_fw_ptr->setup_packet.request] != NULL )
         {
            ...
            if( ( device_state < USB_STATE_POWERED ) &&
                  ( g_validate_request[usb_fw_ptr->setup_packet.request][device_state] == TRUE ) )
            {
               /* Standard Request function pointers */
               error = g_standard_request[usb_fw_ptr->setup_packet.request] ( usb_fw_ptr, &usb_fw_ptr->setup_packet, &data, &size );
            }
         }
      }
      else /* for Class/Vendor requests */
      {
                 ...
                 USB_DATA_TO_DEVICE ) )
         {
            ...
            */
            ...
            ( void )_usb_device_recv_data( event->handle, USB_CONTROL_ENDPOINT, ( usb_fw_ptr->ext_req_to_host + USB_SETUP_PKT_SIZE ), ( uint32_t )( size ) );
            return;
         }
      ...
      USB_Control_Service_Handler( usb_fw_ptr, error, &usb_fw_ptr->setup_packet, &data, &size );
   }
            ...
            ( usb_fw_ptr->setup_packet.length ) )
   {
      ...
      if( usb_fw_ptr->other_req_callback.callback != NULL )
      {
         ...
         error = usb_fw_ptr->other_req_callback.callback( ( USB_SETUP_STRUCT* ) usb_fw_ptr->ext_req_to_host, &data, &size,
                 usb_fw_ptr->other_req_callback.arg );
      }
      USB_Control_Service_Handler( usb_fw_ptr, error, &usb_fw_ptr->setup_packet, &data, &size );
   }
   return;
...
)
{
   if( error == USBERR_INVALID_REQ_TYPE )
   {
      ...
      ( void )_usb_device_set_status( usb_fw_ptr->controller_handle, ( uint8_t )( component | USB_STATUS_ENDPOINT ), ( uint16_t )USB_STATUS_STALLED );
   }
   else /* Need to send Data to the USB Host */
   {
      ...
      ( void )USB_Class_Send_Data( usb_fw_ptr->class_handle, USB_CONTROL_ENDPOINT, *data, *size );
            ...
            USB_DATA_TO_HOST )
      {
         ...
         ( void )_usb_device_recv_data( usb_fw_ptr->controller_handle, USB_CONTROL_ENDPOINT, NULL, 0 );
      }
   }
   return;
}
                                 ...
                                 uint32_t *size )
{
   ...
   {
      /* request for Device */
      error = _usb_device_get_status( usb_fw_ptr->controller_handle, ( uint8_t )USB_STATUS_DEVICE, &usb_fw_ptr->std_framework_data );
             ...
             USB_REQUEST_SRC_INTERFACE )
   {
      ...
      error =  usb_fw_ptr->desc.GET_DESC_INTERFACE( usb_fw_ptr->desc.handle, interface, ( uint8_t * )&usb_fw_ptr->std_framework_data );
             ...
             USB_REQUEST_SRC_ENDPOINT )
   {
      ...
      error =  _usb_device_get_status( usb_fw_ptr->controller_handle, ( uint8_t )endpoint, &usb_fw_ptr->std_framework_data );
      usb_fw_ptr->std_framework_data = SHORT_LE_TO_HOST( usb_fw_ptr->std_framework_data );
                              ...
                              uint32_t *size )
{
   ...
   *size = 0;
         ...
         USB_REQUEST_SRC_DEVICE )
   {
      ...
      {
         /* request for Device */
         error = _usb_device_get_status( usb_fw_ptr->controller_handle, ( uint8_t )USB_STATUS_DEVICE, &device_status );
         ...
         error = _usb_device_set_status( usb_fw_ptr->controller_handle, ( uint8_t )USB_STATUS_DEVICE, ( uint16_t )device_status );
      }
   }
   else if ( ( setup_packet->request_type & USB_REQUEST_SRC_MASK ) ==
             USB_REQUEST_SRC_ENDPOINT ) /*request for Endpoint*/
   {
      ...
      error = _usb_device_set_status( usb_fw_ptr->controller_handle, ( uint8_t )( epinfo | USB_STATUS_ENDPOINT ), ( uint16_t )set_request );
      ...
      {
         usb_fw_ptr->class_callback.callback( event, ( void* )&epinfo, usb_fw_ptr->class_callback.arg );
      }
...
)
{
   ...
   *size = 0;
   /* update device stae */
   ( void )_usb_device_set_status( usb_fw_ptr->controller_handle, ( uint8_t )USB_STATUS_DEVICE_STATE, ( uint16_t )USB_STATE_PENDING_ADDRESS );
...
{
   /* Set Device Address */
   ( void )_usb_device_set_address( usb_fw_ptr->controller_handle, ( uint8_t )( usb_fw_ptr->assigned_address & 0x00FF ) );
   /* Set Device state */
   ( void )_usb_device_set_status( usb_fw_ptr->controller_handle, ( uint8_t )USB_STATUS_DEVICE_STATE, ( uint16_t )USB_STATE_ADDRESS );
   /* Set Device state */
   ( void )_usb_device_set_status( usb_fw_ptr->controller_handle, ( uint8_t )USB_STATUS_ADDRESS, ( uint16_t )usb_fw_ptr->assigned_address );
...
)
{
   ...
   *size = CONFIG_SIZE;
   ( void )_usb_device_get_status( usb_fw_ptr->controller_handle, ( uint8_t )USB_STATUS_CURRENT_CONFIG, &usb_fw_ptr->std_framework_data );
   usb_fw_ptr->std_framework_data = SHORT_LE_TO_HOST( usb_fw_ptr->std_framework_data );
   *data = ( uint8_t * )&usb_fw_ptr->std_framework_data;
...
)
{
   ...
   *size = 0;
   ...
   if ( usb_fw_ptr->desc.IS_DESC_VALID_CONFIGURATION( usb_fw_ptr->desc.handle, config_val ) )
      /*if valid configuration (fn returns bool value)*/
   {
      ...
      error = _usb_device_set_status( usb_fw_ptr->controller_handle, ( uint8_t )USB_STATUS_DEVICE_STATE, ( uint16_t )device_state );
      error = _usb_device_set_status( usb_fw_ptr->controller_handle, ( uint8_t )USB_STATUS_CURRENT_CONFIG, ( uint16_t )config_val );
      /* callback to the app. to let the application know about the new
         Config */
      if ( usb_fw_ptr->class_callback.callback != NULL )
      {
         usb_fw_ptr->class_callback.callback( USB_APP_CONFIG_CHANGED, ( void * )&config_val, usb_fw_ptr->class_callback.arg );
         usb_fw_ptr->class_callback.callback( USB_APP_ENUM_COMPLETE, NULL, usb_fw_ptr->class_callback.arg );
      }
...
)
{
   ...
   *size = INTERFACE_STATUS_SIZE;
   error = usb_fw_ptr->desc.GET_DESC_INTERFACE( usb_fw_ptr->desc.handle, ( uint8_t )setup_packet->index, ( uint8_t * )&usb_fw_ptr->std_framework_data );
   *data = ( uint8_t * )&usb_fw_ptr->std_framework_data;
...
)
{
   ...
   *size = 0;
   ...
   error = usb_fw_ptr->desc.SET_DESC_INTERFACE( usb_fw_ptr->desc.handle, ( uint8_t )setup_packet->index, ( uint8_t )setup_packet->value );
...
)
{
   ...
   *size = FRAME_SIZE;
   ...
   error = _usb_device_get_status( usb_fw_ptr->controller_handle, ( uint8_t )USB_STATUS_SOF_COUNT, &usb_fw_ptr->std_framework_data );
   *data = ( uint8_t * )&usb_fw_ptr->std_framework_data;
...
)
{
   ...
   error = usb_fw_ptr->desc.GET_DESC( usb_fw_ptr->desc.handle, type, str_num, index, data, size );
   return error;
}
/**************************************************************************//*!
...
)
{
   ...
   return usb_fw_ptr->desc.DESC_SET_FEATURE( usb_fw_ptr->desc.handle, cmd, input_data, outBuf );
