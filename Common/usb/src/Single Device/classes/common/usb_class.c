 ...
 *****************************************************************************/
static void USB_Suspend_Service ( PTR_USB_EVENT_STRUCT event, void *arg );
static void USB_Resume_Service ( PTR_USB_EVENT_STRUCT event, void *arg );
static void USB_Stall_Service ( PTR_USB_EVENT_STRUCT event, void *arg );
static void USB_Sof_Service ( PTR_USB_EVENT_STRUCT event, void *arg );
static void USB_Reset_Service ( PTR_USB_EVENT_STRUCT event , void *arg );
static void USB_Error_Service ( PTR_USB_EVENT_STRUCT event , void *arg );
/*****************************************************************************
   Local Variables
 *****************************************************************************/
...
static void USB_Stall_Service ( PTR_USB_EVENT_STRUCT event, void *arg )
{
   ...
   if( event->ep_num == USB_CONTROL_ENDPOINT )
   {
      ...
      ( void )_usb_device_set_status( event->handle,
                                      ( uint8_t )( event->ep_num | USB_STATUS_ENDPOINT ),
                             ...
                             )
{
   ...
   ( void )_usb_device_init_endpoint( event->handle, &ep_struct, TRUE );
   component = ( uint8_t )( ep_struct.ep_num |
                            ( uint8_t )( USB_SEND << COMPONENT_PREPARE_SHIFT ) );
   /* set the EndPoint Status as Idle in the device layer */
   ( void )_usb_device_set_status( event->handle,
                                   ( uint8_t )( USB_STATUS_ENDPOINT | component ),
                                   ( uint16_t )USB_STATUS_IDLE );
...
)
{
   ...
   ( void )_usb_device_get_status( class_object_ptr->controller_handle,
                                   ( uint8_t )USB_STATUS_DEVICE_STATE, &device_state );

   ( void )_usb_device_get_status( class_object_ptr->controller_handle,
                                   ( uint8_t )USB_STATUS_DEVICE, &state );
