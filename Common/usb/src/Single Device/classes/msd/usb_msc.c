 ...
 *****************************************************************************/
uint8_t USB_MSC_Other_Requests( USB_SETUP_STRUCT *setup_packet, uint8_t **data, uint32_t *size, void   *arg );
uint8_t process_mass_storage_command ( MSC_DEVICE_STRUCT_PTR mscDevicePtr, PTR_CBW cbw_ptr, uint32_t* csw_residue_ptr, uint8_t* csw_status_ptr );
 ...
 *****************************************************************************/
static uint32_t USB_Msd_Free_Handle( MSD_HANDLE handle )
{
...
)
{
   ...
   switch ( cbw_ptr->command_block[0] )
   {
      ...
      case INQUIRY_COMMAND : /*opcode : 0x12*/
         error = msc_inquiry_command( msc_obj_ptr, cbw_ptr, csw_residue_ptr, csw_status_ptr );
         break;
      case READ_10_COMMAND : /*opcode : 0x28 */
      case READ_12_COMMAND : /*opcode : 0xA8 */
         error = msc_read_command( msc_obj_ptr, cbw_ptr, csw_residue_ptr, csw_status_ptr );
         break;
      case REQUEST_SENSE_COMMAND : /*opcode : 0x03*/
         error = msc_request_sense_command( msc_obj_ptr, cbw_ptr, csw_residue_ptr, csw_status_ptr );
         break;
      case TEST_UNIT_READY_COMMAND : /*opcode : 0x00 */
         error = msc_test_unit_ready_command( msc_obj_ptr, cbw_ptr, csw_residue_ptr, csw_status_ptr );
         break;
      case WRITE_10_COMMAND : /*opcode : 0x2A */
      case WRITE_12_COMMAND : /*opcode : 0xAA */
         error = msc_write_command( msc_obj_ptr, cbw_ptr, csw_residue_ptr, csw_status_ptr );
         break;
      case PREVENT_ALLOW_MEDIUM_REM_COMMAND : /*opcode :0x1E */
         error = msc_prevent_allow_medium_removal( msc_obj_ptr, cbw_ptr, csw_residue_ptr, csw_status_ptr );
         break;
      case FORMAT_UNIT_COMMAND : /*opcode : 0x04*/
         error = msc_format_unit_command( msc_obj_ptr, cbw_ptr, csw_residue_ptr, csw_status_ptr );
         break;
      case READ_CAPACITY_10_COMMAND : /*opcode : 0x25*/
         error = msc_read_capacity_command( msc_obj_ptr, cbw_ptr, csw_residue_ptr, csw_status_ptr );
         break;
      case MODE_SENSE_10_COMMAND : /* opcode :0x5A*/
      case MODE_SENSE_6_COMMAND : /* opcode : 0x1A */
         error = msc_mode_sense_command( msc_obj_ptr, cbw_ptr, csw_residue_ptr, csw_status_ptr );
         break;
      case MODE_SELECT_10_COMMAND : /*opcode : 0x55 */
      case MODE_SELECT_6_COMMAND : /*opcode : 0x15 */
         error = msc_mode_select_command( msc_obj_ptr, cbw_ptr, csw_residue_ptr, csw_status_ptr );
         break;
      case READ_FORMAT_CAPACITIES_COMMAND : /*opcode : 0x23 */
         error = msc_read_format_capacity_command( msc_obj_ptr, cbw_ptr, csw_residue_ptr, csw_status_ptr );
         break;
      case SEND_DIAGNOSTIC_COMMAND : /*opcode : 0x1D*/
         error = msc_send_diagnostic_command( msc_obj_ptr, cbw_ptr, csw_residue_ptr, csw_status_ptr );
         break;
      case VERIFY_COMMAND : /*opcode : 0x2F*/
         error = msc_verify_command( msc_obj_ptr, cbw_ptr, csw_residue_ptr, csw_status_ptr );
         break;
      case START_STOP_UNIT_COMMAND : /*opcode : 0x1B*/
         error = msc_start_stop_unit_command( msc_obj_ptr, cbw_ptr, csw_residue_ptr, csw_status_ptr );
         break;
      default: /* for all unsupported commands */
         error = msc_unsupported_command( msc_obj_ptr, cbw_ptr, csw_residue_ptr, csw_status_ptr );
...
void USB_Service_Bulk_In( PTR_USB_EVENT_STRUCT event, void *arg )
{
   ...
   if( consumer != producer )
   {
      ...
      ( void )USB_Class_Send_Data( msc_obj_ptr->class_handle, queue.channel, queue.app_data.data_ptr, queue.app_data.data_size );
      return;
   }
#endif
         ...
         ( !msc_obj_ptr->transfer_remaining ) )
   {
      ...
      ( void )_usb_device_set_status( msc_obj_ptr->controller_handle, ( uint8_t )( component | USB_STATUS_ENDPOINT ), ( uint16_t )USB_STATUS_STALLED );
      return;
   }
   ...
   if( msc_obj_ptr->in_flag ) /* bulk in transaction has occured before CSW */
   {
      if( msc_obj_ptr->param_callback.callback != NULL )
      {
         ...
         if( event_type == USB_APP_SEND_COMPLETE )
         {
            ...
            msc_obj_ptr->param_callback.callback( event_type, ( void* )&bulk_in_recv, msc_obj_ptr->param_callback.arg );
         }
         else
         {
            ...
            msc_obj_ptr->param_callback.callback( event_type, ( void* )&lba_data, msc_obj_ptr->param_callback.arg );
            /* Send data on USB Bus */
            ( void )USB_MSC_Bulk_Send_Data( msc_obj_ptr->msc_handle, ( uint8_t * )lba_data.buff_ptr, lba_data.size );
         }
      }
      ...
      if( !msc_obj_ptr->transfer_remaining )
      {
         ...
         ( void )USB_MSC_Bulk_Send_Data( msc_obj_ptr->msc_handle, ( uint8_t * )msc_obj_ptr->csw_ptr, MSC_CSW_LENGTH );
      }
   }
             ...
             && ( csw_ptr->signature == USB_DCSWSIGNATURE ) ) /*valid CSW signature*/
   {
      ...
      ( void )USB_MSC_Bulk_Recv_Data( msc_obj_ptr->controller_handle, ( uint8_t* )msc_obj_ptr->cbw_ptr, MSC_CBW_LENGTH );
   }
}
...
void USB_Service_Bulk_Out( PTR_USB_EVENT_STRUCT event, void *arg )
{
   ...
   if( msc_obj_ptr->out_flag ) /* bulk out transaction has occured after CBW */
   {
      if( msc_obj_ptr->param_callback.callback != NULL )
      {
         ...
         if( event_type == USB_APP_DATA_RECEIVED )
         {
            ...
            msc_obj_ptr->param_callback.callback( event_type, ( void* )&bulk_out_recv, msc_obj_ptr->param_callback.arg );
         }
         else
         {
            ...
            msc_obj_ptr->param_callback.callback( event_type, ( void* )&lba_data1, msc_obj_ptr->param_callback.arg );
         }
      }
      ...
      if( msc_obj_ptr->transfer_remaining )
      {
         ...
         ( void )USB_MSC_Bulk_Recv_Data( msc_obj_ptr->controller_handle, lba_data0.buff_ptr, lba_data0.size );
         return;
      }
      else
      {
   ...
   )
   {
      ...
      msc_obj_ptr->in_flag = ( bool )( ( ( cbw_ptr->flag & USB_CBW_DIRECTION_BIT )
                                         && ( cbw_ptr->data_length ) ) ? TRUE : FALSE );
      /* Process the command */
      error = process_mass_storage_command( msc_obj_ptr, cbw_ptr, &( msc_obj_ptr->csw_ptr->residue ), &( msc_obj_ptr->csw_ptr->csw_status ) );
...
void USB_Class_MSC_Event( uint8_t event, void* val, void *arg )
{
   ...
   if( event == USB_APP_ENUM_COMPLETE )
   {
      ...
      while( count < usb_ep_data->count )
      {
         ...
         uint8_t component = ( uint8_t )( ep_struct_ptr->ep_num |
                                          ( ep_struct_ptr->direction << COMPONENT_PREPARE_SHIFT ) );

         ( void )_usb_device_init_endpoint( msc_dev_ptr->controller_handle, ep_struct_ptr, TRUE );
         ...
         ( void )_usb_device_set_status( msc_dev_ptr->controller_handle, ( uint8_t )( USB_STATUS_ENDPOINT | component ), ( uint16_t )USB_STATUS_IDLE );
         count++;
      }
   ...
   else if( event == USB_APP_EP_UNSTALLED )
   {
              ...
              ( ( value & 0x0F ) == msc_dev_ptr->bulk_out_endpoint ) ) )
      {
         ...
         */
         ( void )_usb_device_set_status( msc_dev_ptr->controller_handle, ( uint8_t )( value | USB_STATUS_ENDPOINT ), ( uint16_t )USB_STATUS_STALLED );
      }
   }
   ...
   {
      msc_dev_ptr->msc_callback.callback( event, val, msc_dev_ptr->msc_callback.arg );
   }
}
 ...
 *****************************************************************************/
uint8_t USB_MSC_Other_Requests
(
...
)
{
   ...
   *size = 0;
         ...
         case BULK_ONLY_MASS_STORAGE_RESET :
                    ...
                    USB_DATA_TO_DEVICE ) )
            {
               ...
               while( count < usb_ep_data->count )
               {
                  ...
                  if( ep_struct_ptr->type == USB_BULK_PIPE )
                  {
                     ( void )_usb_device_deinit_endpoint( msc_dev_ptr->controller_handle, ep_struct_ptr->ep_num, ep_struct_ptr->direction );
                     ( void )_usb_device_init_endpoint( msc_dev_ptr->controller_handle, ep_struct_ptr, TRUE );
            ...
            USB_REQUEST_CLASS_VENDOR )
   {
      ...
      {
         error = msc_dev_ptr->vendor_callback.callback( setup_packet, data, size, msc_dev_ptr->vendor_callback.arg );
      }
   }
...
)
{
   ...
   USB_mem_copy( &msd_config_ptr->device_info, &devicePtr->device_info, sizeof( DEVICE_LBA_INFO_STRUCT ) );
   USB_mem_copy( &msd_config_ptr->msd_buff, &devicePtr->msd_buff, sizeof( MSD_BUFF_INFO ) );
   devicePtr->lun = ( uint8_t )( msd_config_ptr->device_info.num_lun_supported - 1 );

   /* Initialize the device layer*/
   error = _usb_device_init( USBCFG_DEFAULT_DEVICE_CONTROLLER, &devicePtr->controller_handle, ( uint8_t )( usb_ep_data->count + 1 ) );
   ...
   devicePtr->class_handle = USB_Class_Init( devicePtr->controller_handle, USB_Class_MSC_Event, USB_MSC_Other_Requests, ( void * )devicePtr, msd_config_ptr->desc_callback_ptr );

   /* Initialize the pstn subclass functions */
   scsi_cb.callback = USB_Class_MSC_Event;
   scsi_cb.arg   = devicePtr;
   error = USB_MSC_SCSI_Init( devicePtr, &scsi_cb, &msd_config_ptr->device_info, msd_config_ptr->implementing_disk_drive );
