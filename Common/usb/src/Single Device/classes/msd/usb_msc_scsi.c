...
uint8_t msc_thirteen_cases_check( MSC_DEVICE_STRUCT_PTR msc_obj_ptr, PTR_MSC_THIRTEEN_CASE_STRUCT msc_check_event )
{
   ...
   else if( msc_check_event->host_expected_direction )
   {
      ...
      if( !msc_check_event->device_expected_data_len )
      {
         ...
         *( msc_check_event->csw_residue_ptr ) = msc_check_event->host_expected_data_len - msc_check_event->device_expected_data_len;
         /* sending zero bytes of data */
         error = USB_MSC_Bulk_Send_Data( msc_obj_ptr->msc_handle, msc_check_event->buffer_ptr, msc_check_event->device_expected_data_len );
      ...
      else if( msc_check_event->device_expected_direction )
      {
               ...
               msc_check_event->device_expected_data_len )
         {
            ...
            *( msc_check_event->csw_residue_ptr ) = msc_check_event->host_expected_data_len - msc_check_event->device_expected_data_len;
            ...
            {
               error = USB_MSC_LBA_Transfer( msc_obj_ptr, USB_SEND, &msc_check_event->lba_info );
            }
            else
            {
               error = USB_MSC_Bulk_Send_Data( msc_obj_ptr->msc_handle, msc_check_event->buffer_ptr, msc_check_event->device_expected_data_len );
                  ...
                  msc_obj_ptr->bulk_in_endpoint_packet_size == 0 )
            {
               ...
               error |= USB_MSC_Bulk_Send_Data( msc_obj_ptr->msc_handle, msc_check_event->buffer_ptr, 0 );
            }
         ...
         else if( msc_check_event->host_expected_data_len == msc_check_event->device_expected_data_len )
         {
         ...
         else
         {
            ...
            *( msc_check_event->csw_residue_ptr ) = 0;
            ...
            {
               error = USB_MSC_Bulk_Send_Data( msc_obj_ptr->msc_handle, msc_check_event->buffer_ptr, msc_check_event->host_expected_data_len );
            }
      ...
      else
      {
         ...
         *( msc_check_event->csw_residue_ptr ) = msc_check_event->host_expected_data_len;
      ...
      else
      {
               ...
               msc_check_event->device_expected_data_len )
         {
            ...
            *( msc_check_event->csw_residue_ptr ) = msc_check_event->host_expected_data_len - msc_check_event->device_expected_data_len;
            ...
            if( scsi_ptr->thirteen_case_ptr->lba_txrx_select == TRUE )
            {
               error = USB_MSC_LBA_Transfer( msc_obj_ptr, USB_RECV, &msc_check_event->lba_info );
            }
            else
            {
               error = USB_MSC_Bulk_Recv_Data( msc_obj_ptr->controller_handle, msc_check_event->buffer_ptr, msc_check_event->device_expected_data_len );
            }
         ...
         else
         {
            ...
            *( msc_check_event->csw_residue_ptr ) = 0;
            ...
            if( scsi_ptr->thirteen_case_ptr->lba_txrx_select == TRUE )
            {
               msc_check_event->lba_info.lba_transfer_num = msc_check_event->host_expected_data_len / scsi_ptr->length_of_each_lab;
               error = USB_MSC_LBA_Transfer( msc_obj_ptr, USB_RECV, &msc_check_event->lba_info );
            }
            else
            {
               error = USB_MSC_Bulk_Recv_Data( msc_obj_ptr->controller_handle, msc_check_event->buffer_ptr, msc_check_event->host_expected_data_len );
            }
...
)
{
   ...
   scsi_ptr->total_logical_add_block = device_info_ptr->total_lba_device_supports;
   scsi_ptr->length_of_each_lab = device_info_ptr->length_of_each_lab_of_device;
   ...
   scsi_ptr->thirteen_case_ptr = ( PTR_MSC_THIRTEEN_CASE_STRUCT )USB_mem_alloc_zero( sizeof( MSC_THIRTEEN_CASE_STRUCT ) );
...
)
{
   ...
   scsi_ptr->thirteen_case_ptr->host_expected_direction = ( uint8_t )( cbw_ptr->flag >> USB_CBW_DIRECTION_SHIFT );
...
)
{
   ...
   scsi_ptr->thirteen_case_ptr->device_expected_data_len = scsi_ptr->length_of_each_lab * num_lba_tx;
...
)
{
   ...
   if( _usb_device_get_transfer_status( msc_obj_ptr->controller_handle, BULK_OUT_ENDPOINT, USB_RECV ) != USB_STATUS_IDLE )
   {
      ( void )_usb_device_cancel_transfer( msc_obj_ptr->controller_handle, BULK_OUT_ENDPOINT, USB_RECV );
   }

   scsi_ptr->thirteen_case_ptr->device_expected_data_len = scsi_ptr->length_of_each_lab * num_lba_rx;
...
)
{
   ...
   */
   ...
   response_size = sizeof( capacity_list_header ) + sizeof( curr_max_cap_header ) + sizeof( formattable_cap_descriptor ) * num_formattable_cap_desc;
   ...
   USB_mem_copy( &capacity_list_header, response_data_ptr, sizeof( capacity_list_header ) );
   USB_mem_copy( &curr_max_cap_header, response_data_ptr + sizeof( capacity_list_header ), sizeof( curr_max_cap_header ) );
...
)
{
   ...
   prevent_removal = ( uint8_t )( cbw_ptr->command_block[4] & PREVENT_ALLOW_REMOVAL_MASK );
   ...
   {
      scsi_ptr->scsi_callback.callback( USB_MSC_DEVICE_REMOVAL_REQUEST, ( void* )&prevent_removal, scsi_ptr->scsi_callback.arg );
   }
...
)
{
   ...
   /* masking to obtain value of last bit */
   load_eject_start = ( uint8_t )( cbw_ptr->command_block[4] & LOAD_EJECT_START_MASK );
   /* initialize sense code values */
   scsi_ptr->request_sense.sense_key = NO_SENSE;
   scsi_ptr->request_sense.add_sense_code = NO_SENSE;
   scsi_ptr->request_sense.add_sense_code_qual = NO_SENSE;
   ...
   if( *csw_status_ptr != PHASE_ERROR )
   {
      if( scsi_ptr->scsi_callback.callback != NULL )
      {
         scsi_ptr->scsi_callback.callback( USB_MSC_START_STOP_EJECT_MEDIA, ( void* )&load_eject_start, scsi_ptr->scsi_callback.arg );
      }
   }
   return error;
}
